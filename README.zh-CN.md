...
Andrias 是面向模型应用的能力运行时。它把模型推理、工具调用、记忆、状态和外部系统映射为
`effect://` 与 `state://` 资源。程序通过 `open()` 编译出的句柄访问这些资源。模型后端、工具进程、状态存储和外部程序分别位于资源绑定、driver 或
Provider/Source projection 后面。

Andrias 负责模型调用、工具执行、长期状态和外部协议之间的执行边界。模型训练、应用界面和业务逻辑在运行时外部。所有访问都经过同一套 `Resource / Interface / Driver`、`open()` 句柄和能力检查路径。
...
核心运行时和 external Provider/Source gateway 已经实现。Andrias 还没有发布第一版，当前 gateway/config/protobuf 契约由 external Provider/Source 模型定义。
...
| Rust 包 | 职责 |
| --- | --- |
| `andrias-types` | 核心 ID、路径、值、能力、操作、审计类型和 external Provider/Source 数据。 |
| `andrias-graph` | 可恢复 `Do<A>` 程序中间表示和执行图编译器。 |
| `andrias-state` | 状态后端 trait 与内存实现。 |
| `andrias-kernel` | 进程、句柄、注册表、策略、执行器、恢复、事实记录。 |
| `andrias-storage-redb` | 基于 redb 的持久状态和 `FactStore`。 |
| `andrias-standard` | 标准进程内 Provider 和 Source 实现。 |
| `andrias-gateway` | external 协议适配器共享的 session 准入、流控、taint 和 audit 代码。 |
| `andrias-gateway-grpc` | 使用 `andrias-proto` 的 external Provider/Source gRPC 适配器。 |
| `andrias-gateway-websocket` | external Provider/Source WebSocket 适配器。 |
| `andrias-gateway-mcp` | 把选定 Gateway publication 暴露为 MCP tool、resource、resource template 和 prompt 的服务端适配器。 |
| `andrias-proto` | Protobuf schema 和随仓库提供的 prost/tonic 绑定。 |
| `andrias-console` | Web 控制台管理动作的 gateway。 |
| `andrias-daemon` | 长期运行的宿主进程 `andriasd`。 |
| `andrias-sdk` | 最小嵌入式运行时门面和便捷导出。 |
| `andrias-plan`, `andrias-sim` | 规划与仿真 crate。 |

`andrias-standard` 使用 Cargo feature 选择要编译的模块。默认 `standard` 会编译标准进程内
...
`andrias-sdk` 默认只构建最小内存内核。嵌入式宿主通过 `AndriasBuilder` 提供自己的状态后端
和事实记录接收端。标准 Provider 通过 SDK 的 `standard` feature 或 `andrias-standard`
安装入口接入。
`ActorSpec` 可通过 `andrias-graph` 和 `andrias-sdk` 使用；内核可以通过
`Bootstrap::spawn_actor_under` 把它启动为命名长寿 Process，SDK 也提供
`Andrias::spawn_actor` 便捷入口。body 或终结器如果引用进程本地 `StepRef`，宿主应在启动时
通过 `Bootstrap::spawn_actor_under_with_steps` 或 `Andrias::spawn_actor_with_steps`
...
- Rust 1.95 或更新版本。
- 与稳定版工具链匹配的 Cargo。
- 不需要本地安装 `protoc`。`andrias-proto` 已包含随仓库提供的 prost/tonic Rust 绑定。
...
`andriasd` 只启动宿主进程；运行时管理通过 Console WebSocket 执行。

`andrias.toml.example` 默认地址：

- 控制台监听地址：`127.0.0.1:9000`
- External gRPC gateway：`127.0.0.1:9444`
- External WebSocket gateway：`127.0.0.1:9200`

`andrias-daemon` 默认启用 `external-grpc` 和 `external-websocket`。可以用
`--no-default-features --features external-grpc` 或
`--no-default-features --features external-websocket` 只构建其中一种
transport。

首次启动时，如果状态中没有控制台 root 账号，也没有配置预置凭据，`andriasd` 会把一次性 root 密码打印到 stderr。

## 配置

`andrias.toml` 是引导配置。它控制存储、监听绑定地址、root 引导凭据、external gateway 限制、gateway 传输安全设置和控制台资源上限。
...
运行时配置、Provider 设置、模型路由、组、绑定、外部程序安装、Provider projection、Source projection 和策略管理状态都属于 Andrias state，并通过 Console WebSocket 管理。
...
`andrias-console` 是 Web 控制台管理动作的 gateway。HTTP 负责健康检查和认证；登录后，Console WebSocket 承载快照、配置读取/写入/CAS、运行时检查、订阅、trace/fact stream、外部程序生命周期动作、配对动作、登出和控制台用户/角色/会话管理。

### External Gateway

外部程序以 Provider 或 Source projection 接入。daemon 持有 session 准入、generation 检查、流控、去重、command idempotency 和入站污点标记。

External gRPC 通过 `[server].external_grpc_addr` 或 `ANDRIAS_EXTERNAL_GRPC_ADDR` 提供 Provider/Source session stream。

External WebSocket 通过 `[server].external_websocket_addr` 或 `ANDRIAS_EXTERNAL_WEBSOCKET_ADDR` 提供同一套 Provider/Source session frame。
