...
//! `andrias-sdk` — embedded façade.
//!
//! Re-exports the embedded kernel surface and provides [`AndriasBuilder`] plus a
//! small [`Andrias`] wrapper for applications embedding the kernel in-process.

pub use andrias_graph::{
    ActorSpec, CapabilityQueryError, DoNode, ExecutionGraph, GraphCursor, NodeKind,
    OperationTemplate, StepRef, compile_do, lint_actor,
};
pub use andrias_kernel::{
...
pub use andrias_plan::{Plan, PlanError, Step, WriteModeSpec, compile_for, parse_json, parse_yaml};
#[cfg(feature = "standard")]
pub use andrias_standard::{
...
pub fn andrias() -> Bootstrap {
    AndriasBuilder::new().build_bootstrap()
}

#[cfg(feature = "standard")]
/// Build an in-memory [`Bootstrap`] and install the standard in-process package.
pub fn andrias_with_standard(config: &StandardConfig) -> Result<Bootstrap, InstallError> {
    AndriasBuilder::new().build_bootstrap_with_standard(config)
}

/// Errors returned by embedded program execution helpers.
#[derive(Debug)]
pub enum AndriasError {
    /// A resource path supplied to [`Andrias::run`] was malformed.
    Path {
...
impl fmt::Display for AndriasError {
...
impl AndriasBuilder {
    ...
    pub fn build(self) -> Andrias {
        Andrias::from_bootstrap(self.build_bootstrap())
    }

    #[cfg(feature = "standard")]
    /// Build an embedded runtime wrapper with the standard package installed.
    pub fn build_with_standard(self, config: &StandardConfig) -> Result<Andrias, InstallError> {
        Ok(Andrias::from_bootstrap(
...
impl Andrias {
    ...
    pub fn with_standard(config: &StandardConfig) -> Result<Self, InstallError> {
        AndriasBuilder::new().build_with_standard(config)
    }
    ...
    pub async fn run(&self, resources: &[&str], program: DoNode) -> Result<Outcome, AndriasError> {
        ...
        let process = self
            ...
            .map_err(|source| AndriasError::Bootstrap { source })?;
    ...
    pub async fn run_plan(&self, resources: &[&str], plan: &Plan) -> Result<Outcome, AndriasError> {
        let grants = self.request_grants(resources)?;
        let program =
            compile_for(self.boot.root, plan).map_err(|source| AndriasError::Plan { source })?;
        let process = self
            .boot
            .spawn_request_process_under_with_compiled_request_grants(
                self.boot.root,
        ...
        let program =
            program
                ...
                .map_err(|source| AndriasError::Resource {
    ...
    fn request_grants(
        &self,
        resources: &[&str],
    ) -> Result<Vec<andrias_kernel::CompiledRequestGrantTemplate>, AndriasError> {
        resources
            .iter()
            .map(|resource| {
                let path = Path::parse(resource).map_err(|source| AndriasError::Path {
                    resource: (*resource).to_string(),
                    source,
                })?;
                if path.cluster().is_some() {
                    return Err(AndriasError::Resource {
                        resource: (*resource).to_string(),
                ...
                let methods =
                    self.boot
                        ...
                        .map_err(|source| AndriasError::Open {
                            resource: (*resource).to_string(),
                            source,
                        })?;
                let literal = capability_literal("*", &path);
                let selector = andrias_types::ResourceSelector::parse(&literal).map_err(|source| {
                    AndriasError::Resource {
                        resource: (*resource).to_string(),
                        reason: source.to_string(),
                    }
                })?;
                Ok(andrias_kernel::CompiledRequestGrantTemplate {
                    selector,
                    methods,
                })
            })
...
mod tests {
    ...
    async fn minimal_constructor_does_not_install_standard_providers() -> anyhow::Result<()> {
        ...
        let err = nx.run(&["effect://time/now"], prog).await;
            ...
            matches!(err, Err(AndriasError::Open { .. })),
    ...
    async fn builder_uses_supplied_state_backend() -> anyhow::Result<()> {
        ...
        let nx = AndriasBuilder::new().with_state_backend(state).build();
    ...
    #[tokio::test]
    async fn andrias_spawn_actor_runs_under_root() -> anyhow::Result<()> {
        let nx = Andrias::new();
        let spec = ActorSpec {
            name: "worker".into(),
    ...
    #[tokio::test]
    async fn andrias_spawn_actor_with_steps_installs_before_run() -> anyhow::Result<()> {
        let nx = Andrias::new();
        let spec = ActorSpec {
    ...
    async fn standard_constructor_installs_standard_providers() -> anyhow::Result<()> {
        let nx = Andrias::with_standard(&StandardConfig::default())?;
        let prog = DoNode::Op(OperationTemplate {
            target: ResourceName::new(p("effect://time/now")?),
            method: "invoke".into(),
            method_id: None,
