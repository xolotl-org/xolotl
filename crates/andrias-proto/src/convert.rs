//! Lossless conversions between `andrias_types` (kernel-side) and the generated
//! protobuf wire types (`crate::andrias::v1`). These are the canonical
...
use andrias_graph::{DoNode, OperationTemplate, StepRef, WaitSpec};
use andrias_types::{
...
/// `andrias_types::Value` → wire `Value`. Total and lossless.
...
/// Wire `Value` → `andrias_types::Value`. An absent oneof maps to `Null`.
...
/// Wire `Value` → `andrias_types::Value` with canonical field validation.
...
pub fn capability_from_pb(c: &pb::Capability) -> Result<Capability, ConvertError> {
    let predicate = c
        .predicate
        .as_deref()
        .map(andrias_types::Predicate::parse)
        .transpose()?;
...
//
// The `andrias_types::external` frames ⇄ the generated `pb::external` wire
...
) -> andrias_types::external::ObservedGenerations {
    andrias_types::external::ObservedGenerations {
        presentation_config_generation: observed.presentation_config_generation,
        alias_catalog_generation: observed.alias_catalog_generation,
    }
