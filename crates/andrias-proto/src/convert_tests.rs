...
fn pb_path(path: &str) -> anyhow::Result<crate::andrias::v1::Path> {
...
fn pb_blob() -> crate::andrias::v1::BlobRef {
    let blob = blob();
    crate::andrias::v1::BlobRef {
        hash: blob.hash,
        size: blob.size,
        mime: blob.mime,
    }
...
fn external_value_frames_reject_malformed_multimodal_refs() -> anyhow::Result<()> {
    use crate::andrias::v1 as pb;
    use crate::andrias::v1::external as ext;
...
fn program_explicit_unspecified_output_mode_fails_closed() -> anyhow::Result<()> {
    ...
    let crate::andrias::v1::do_node::Kind::Op(op) =
        root.kind.as_mut().context("missing root kind")?
    else {
        bail!("expected op node");
    };
    op.output = Some(crate::andrias::v1::OutputMode {
        kind: crate::andrias::v1::OutputModeKind::Unspecified as i32,
        collect_limit: 0,
    });
...
fn program_blank_failure_kind_fails_closed() -> anyhow::Result<()> {
    ...
    let root = program.root.as_mut().context("missing program root")?;
    let crate::andrias::v1::do_node::Kind::Fail(failure) =
        root.kind.as_mut().context("missing root kind")?
    else {
...
fn program_missing_root_fails_closed() {
    let program = crate::andrias::v1::Program {
        root: None,
        provenance: None,
    };
    assert!(program_from_pb(&program).is_err());
...
fn outcome_done_and_fail_map() -> anyhow::Result<()> {
    ...
    let pb = outcome_to_pb(&done);
        ...
        matches!(
            pb.result,
            Some(crate::andrias::v1::outcome::Result::Done(_))
        ),
    ...
    match pb.result {
        Some(crate::andrias::v1::outcome::Result::Fail(f)) => {
...
fn value_survives_protobuf_encode_decode() -> anyhow::Result<()> {
    ...
    let decoded = crate::andrias::v1::Value::decode(&bytes[..])?;
...
fn external_control_frames_roundtrip_through_pb() -> anyhow::Result<()> {
    ...
    use andrias_types::Value;
    use andrias_types::external::{
        ApplyStatus, ConfigAxis, ControlFrame, FlowSignal, RejectReason,
    };
...
fn service_names_and_method_paths_are_correct() {
    assert_eq!(
        crate::andrias::v1::external::external_service_server::SERVICE_NAME,
        "andrias.v1.external.ExternalService"
    );
}
...
fn invoke_frame_types_roundtrip_through_pb() -> anyhow::Result<()> {
    ...
    use andrias_types::external::Invoke;
    use andrias_types::{MethodId, Value};
    let inv = Invoke {
...
fn invoke_result_ok_and_err_roundtrip() -> anyhow::Result<()> {
    ...
    use andrias_types::Value;
    use andrias_types::external::{ErrorInfo, InvokeResult};
    let ok = InvokeResult {
...
fn source_command_frames_roundtrip() -> anyhow::Result<()> {
    ...
    use andrias_types::Value;
    use andrias_types::external::{CommandResult, ErrorInfo, ObservedGenerations, OutboundCommand};
