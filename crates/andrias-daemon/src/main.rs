...
//! `andriasd` - the long-running Andrias host process and bootstrap command line.
//!
//! `andriasd` is both the command you run and the process that runs: like
...
//!
//! Usage:
//!   andriasd                 launch the host (default)
//!   andriasd up              launch the host (explicit)
//!   andriasd info            print version / build info and exit
//!   andriasd --version | -V  print the version and exit
//!   andriasd --help | -h     print usage and exit
//!
...
use andrias_gateway::GatewayTransportSecurityConfig;
#[cfg(all(test, feature = "external-gateway"))]
use andrias_gateway::external::SourceCommandRegister;
#[cfg(feature = "external-gateway")]
...
use andrias_gateway_websocket::{ExternalWebSocketConfig, ExternalWebSocketService};
#[cfg(feature = "external-gateway")]
use andrias_kernel::PolicySnapshot;
#[cfg(feature = "external-gateway")]
use andrias_kernel::driver::{DriverDescriptor, DriverError, RemoteEndpoint, RemoteInvokeDispatch};
#[cfg(feature = "external-gateway")]
use andrias_kernel::{EchoDriver, Registry, ResolveError};
use andrias_sdk::{Backend, Bootstrap, FactSink, Kernel};
use andrias_standard::{
...
use andrias_state::StateEvent;
use andrias_storage_redb::RedbStore;
#[cfg(all(test, feature = "external-gateway"))]
use andrias_types::external::ObservedGenerations;
#[cfg(all(test, feature = "external-gateway"))]
use andrias_types::external::OutboundCommand;
#[cfg(feature = "external-gateway")]
...
#[cfg(feature = "external-gateway")]
use andrias_types::{IdentityRef, ResourceId};
use andrias_types::{InProcessProjectionPhase, InProcessProjectionStatus, Path, Value};
use anyhow::Result;
use config::AndriasConfig;
#[cfg(feature = "external-gateway")]
...
const CONSOLE_ADDR_ENV: &str = "ANDRIAS_CONSOLE_ADDR";
#[cfg(feature = "external-websocket")]
const EXTERNAL_WEBSOCKET_ADDR_ENV: &str = "ANDRIAS_EXTERNAL_WEBSOCKET_ADDR";
#[cfg(feature = "external-grpc")]
const EXTERNAL_GRPC_ADDR_ENV: &str = "ANDRIAS_EXTERNAL_GRPC_ADDR";

const USAGE: &str = "\
andriasd — the Andrias host process

Usage:
  andriasd [up]            launch the host (default)
  andriasd info            print version / build info and exit
  andriasd --version, -V   print the version and exit
  andriasd --help, -h      print this help and exit
...
fn main() -> ExitCode {
    match dispatch_main() {
        Ok(code) => code,
        Err(error) => {
            if let Err(stderr_error) = write_stderr_line(format_args!("andriasd: {error:#}")) {
...
fn dispatch_main() -> Result<ExitCode> {
    ...
    match std::env::args().nth(1).as_deref() {
        Some("--version") | Some("-V") => {
            write_stdout_line(format_args!("andriasd {}", env!("CARGO_PKG_VERSION")))?;
            Ok(ExitCode::SUCCESS)
        }
        Some("--help") | Some("-h") => {
        ...
        Some(other) => {
            write_stderr_line(format_args!(
                "andriasd: unknown command '{other}'\n\n{USAGE}"
            ))?;
            Ok(ExitCode::from(2))
        }
    }
...
async fn serve() -> Result<()> {
    ...
    tracing::info!(version = env!("CARGO_PKG_VERSION"), "starting andriasd");

    let cfg = AndriasConfig::load()?.unwrap_or_default();
    ...
    let (state, facts): (Backend, FactSink) = match cfg.storage.kind.as_str() {
        "memory" => {
            ...
            (
                Arc::new(andrias_sdk::InMemoryBackend::new()),
    ...
    if andrias_console::root_random_password_needed(&boot, &root_provisioning).await?
        && !std::io::stderr().is_terminal()
    {
        anyhow::bail!(
            "refusing to bootstrap root with a random password because stderr is not a TTY; set console.root.password_hash or console.root.pubkeys in andrias.toml"
        );
    }
    let root_bootstrap = andrias_console::bootstrap_root_account(&boot, root_provisioning).await?;
    ...
    if let Some(addr) = console_addr {
        ...
        handles.push(tokio::spawn(async move {
            if let Err(e) = andrias_console::serve(listener, state).await {
...
fn in_process_projection_watch_pattern() -> Result<Path, andrias_types::PathError> {
    Path::try_new("state")?
        .try_push("kernel")?
...
fn projection_error_role(error: &InstallError) -> Option<andrias_types::Role> {
    match error {
...
fn write_bootstrap_credentials(username: &str, password: &str) -> std::io::Result<()> {
    write_stderr_line(format_args!(""))?;
    write_stderr_line(format_args!("Andrias Console bootstrap account created"))?;
...
async fn start_external_websocket(
    ...
    handles.push(tokio::spawn(async move {
        if let Err(e) = andrias_gateway_websocket::serve(service, listener).await {
...
async fn start_external_grpc(
    ...
    let service = andrias_gateway_grpc::ExternalGrpcService::with_config(
        handler,
        andrias_gateway_grpc::ExternalGrpcConfig {
...
struct ProviderSessionRecord {
    endpoint_id: andrias_types::EndpointId,
    context: SessionContext,
...
#[cfg(feature = "external-gateway")]
fn external_state_path(segments: &[&str]) -> Result<Path, andrias_types::PathError> {
...
mod tests {
    use super::*;
    #[cfg(feature = "external-grpc")]
    use andrias_gateway::external::EnvelopeAad;
    ...
    async fn projection_status_reconcile_deletes_stale_entries() -> anyhow::Result<()> {
        ...
        write_in_process_projection_status(
            ...
            &InProcessProjectionStatus {
                ...
                role: Some(andrias_types::Role::Provider),
        ...
        let installed = InProcessProjectionInstalled {
            ...
            role: andrias_types::Role::Provider,
            version: 1,
        };
    ...
    fn external_outbound_channel() -> (
        ...
        tokio::sync::mpsc::Receiver<
            Result<andrias_proto::andrias::v1::external::ExternalFrame, tonic::Status>,
        >,
    ) {
    ...
    async fn recv_external_frame(
        ...
        >,
        label: &'static str,
    ) -> anyhow::Result<andrias_proto::andrias::v1::external::ExternalFrame> {
        rx.recv()
            .await
    ...
    fn expect_outbound_command_frame(
        frame: andrias_proto::andrias::v1::external::ExternalFrame,
    ) -> anyhow::Result<andrias_proto::andrias::v1::external::OutboundCommand> {
        match frame.frame {
            Some(andrias_proto::andrias::v1::external::external_frame::Frame::OutboundCommand(
                command,
            )) => Ok(command),
    ...
    fn expect_invoke_frame(
        frame: andrias_proto::andrias::v1::external::ExternalFrame,
    ) -> anyhow::Result<andrias_proto::andrias::v1::external::Invoke> {
        match frame.frame {
            Some(andrias_proto::andrias::v1::external::external_frame::Frame::Invoke(invoke)) => {
                Ok(invoke)
            }
    ...
    fn expect_control_frame(
        frame: andrias_proto::andrias::v1::external::ExternalFrame,
    ) -> anyhow::Result<andrias_proto::andrias::v1::external::ControlFrame> {
        match frame.frame {
            Some(andrias_proto::andrias::v1::external::external_frame::Frame::Control(control)) => {
                Ok(control)
            }
    ...
    impl TestExternalOutbound {
        async fn send_frame(
            &self,
            frame: andrias_proto::andrias::v1::external::external_frame::Frame,
        ) -> Result<(), tonic::Status> {
            self.tx
                .send(Ok(andrias_proto::andrias::v1::external::ExternalFrame {
                    frame: Some(frame),
                }))
                .await
    ...
    impl ExternalSessionOutbound for TestExternalOutbound {
        ...
        async fn send_invoke(&self, invoke: Invoke) -> Result<(), Self::Error> {
            self.send_frame(
                andrias_proto::andrias::v1::external::external_frame::Frame::Invoke(
                    andrias_proto::invoke_to_pb(&invoke),
                ),
            )
            .await
        }
        ...
        async fn send_outbound_command(&self, command: OutboundCommand) -> Result<(), Self::Error> {
            self.send_frame(
                andrias_proto::andrias::v1::external::external_frame::Frame::OutboundCommand(
                    andrias_proto::outbound_command_to_pb(&command),
                ),
            )
            .await
        }
        ...
        async fn send_control(&self, frame: ControlFrame) -> Result<(), Self::Error> {
            self.send_frame(
                andrias_proto::andrias::v1::external::external_frame::Frame::Control(
                    andrias_proto::control_frame_to_pb(&frame),
                ),
            )
            .await
        }
    ...
    fn hello_from_context(context: &SessionContext) -> RoleSessionClientHello {
        RoleSessionClientHello {
            ...
            observed: andrias_types::external::ObservedGenerations {
                presentation_config_generation: context.presentation_config_generation,
                alias_catalog_generation: context.alias_catalog_generation,
            },
            config_schema: None,
    ...
    async fn ready_fixture(
        ...
        session
            .on_ready(&andrias_types::external::RoleReady {
    ...
    async fn daemon_external_handler_ingests_source_event() -> anyhow::Result<()> {
        ...
        assert_eq!(ack.status, andrias_types::external::AckStatus::Accepted);
        let rows = boot
            .kernel
            .state
    ...
    async fn daemon_external_handler_gates_external_control_frames() -> anyhow::Result<()> {
        ...
        let err = match ExternalSessionHandler::on_control(
            &handler,
            ControlFrame::FlowControl(andrias_types::external::FlowSignal::Pause),
            &session,
            context.clone(),
        )
        .await
    ...
    async fn daemon_external_handler_resolves_only_registered_source_commands() -> anyhow::Result<()>
    {
        ...
        let command = andrias_types::external::OutboundCommand {
            id: "cmd-1".into(),
        ...
        let sent = andrias_proto::outbound_command_from_pb(&sent)
        ...
        let err = match handler
            .send_source_command(
                andrias_types::external::OutboundCommand {
        ...
        let err = match ExternalSessionHandler::on_command_result(
            ...
            CommandResult {
                id: "cmd-large-error".into(),
                outcome: Err(andrias_types::ErrorInfo {
                    kind: "remote".into(),
                    message: "x"
    ...
    async fn daemon_external_handler_expires_pending_source_commands() -> anyhow::Result<()> {
        ...
        assert_eq!(
            andrias_proto::outbound_command_from_pb(&sent)
    ...
    -> anyhow::Result<()> {
        ...
        let first = andrias_types::external::Invoke {
            invocation_id: "invoke-1".into(),
            effect_path: parse_test_path("effect://external-provider/chat/search")?,
            method_id: andrias_types::MethodId::new(0),
            input: Value::Str("query".into()),
        ...
        let second = andrias_types::external::Invoke {
    ...
    fn daemon_provider_binding_relink_keeps_resource_id() -> anyhow::Result<()> {
        ...
        let declaration = ProviderBindingDeclaration {
            path: path.clone(),
            purity: andrias_types::Purity::Effectful,
    ...
    async fn daemon_ready_provider_routes_declared_remote_endpoint_binding() -> anyhow::Result<()> {
        ...
        let sent =
            andrias_proto::invoke_from_pb(&sent).context("decoding provider invoke frame")?;
    ...
    async fn daemon_provider_endpoint_times_out_pending_invocation() -> anyhow::Result<()> {
        ...
        let sent =
            andrias_proto::invoke_from_pb(&sent).context("decoding timed provider invoke frame")?;
        ...
        let control = andrias_proto::control_frame_from_pb(&control)
