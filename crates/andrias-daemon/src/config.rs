...
/// Bootstrap-only config loaded by `andriasd`: storage, listeners, console root
...
/// groups, routing, bindings, process declarations, and policies lives in Andrias
/// state.
#[derive(Debug, Clone, Deserialize, Default)]
#[serde(deny_unknown_fields)]
pub struct AndriasConfig {
    #[serde(default)]
...
impl AndriasConfig {
    pub fn load() -> Result<Option<Self>> {
        let path = std::env::var("ANDRIAS_CONFIG").unwrap_or_else(|_| "andrias.toml".into());
        ...
        let config: AndriasConfig = toml::from_str(&content)
...
mod tests {
    ...
    fn assert_config_rejects_unknown_field(toml: &str, field: &str) -> anyhow::Result<()> {
        let err = match toml::from_str::<AndriasConfig>(toml) {
    ...
    fn temp_config_file(name: &str, contents: &[u8]) -> anyhow::Result<String> {
        ...
        let path = std::env::temp_dir().join(format!(
            "andrias-config-test-{}-{nanos}-{name}",
            std::process::id()
        ));
    ...
    fn config_file_accepts_console_transport_security() -> anyhow::Result<()> {
        let cfg: AndriasConfig = toml::from_str(
            r#"
...
"#,
    ...
    fn console_plain_listener_rejects_production_tls() -> anyhow::Result<()> {
        let cfg = toml::from_str::<AndriasConfig>(
            r#"
[console.transport_security]
mode = "production_tls"
"#,
...
"#,
    ...
    fn example_config_uses_declared_fields() -> anyhow::Result<()> {
        let path = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../andrias.toml.example");
        let content = std::fs::read_to_string(&path)
            .with_context(|| format!("reading example config {}", path.display()))?;
        toml::from_str::<AndriasConfig>(&content)
...
"#,
    ...
    fn external_gateway_plain_listener_tls_modes_fail_closed() -> anyhow::Result<()> {
        ...
        let cfg: AndriasConfig = toml::from_str(&format!(
            r#"
[external_gateway.websocket.transport_security]
mode = "production_tls"
