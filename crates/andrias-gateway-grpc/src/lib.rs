...
//! `andrias-gateway-grpc` - gRPC protocol adapter.
//!
//! Tonic adapter for external Provider and Source sessions.
//!
//! [`ExternalGrpcService`] serves `andrias.v1.external.ExternalService.Session`.
//! Values are marshalled with the structural `andrias-proto` conversions.
//!
//! Builds anywhere prost/tonic resolve - `andrias-proto`'s bindings are vendored,
//! so no `protoc` is required.

use andrias_gateway::GatewayTransportSecurityConfig;
use andrias_gateway::external::ExternalSessionHandler;
use andrias_proto::andrias::v1::external as ext;
use andrias_proto::andrias::v1::external::external_service_server::{
...
/// gRPC adapter for `andrias.v1.external.ExternalService.Session`.
#[derive(Clone)]
