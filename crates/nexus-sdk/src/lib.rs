...
impl Nexus {
    ...
    pub async fn run(&self, resources: &[&str], program: DoNode) -> Result<Outcome, NexusError> {
        ...
        let process = self
            ...
            .map_err(|source| NexusError::Bootstrap { source })?;
    ...
    pub async fn run_plan(&self, resources: &[&str], plan: &Plan) -> Result<Outcome, NexusError> {
        ...
        let program =
            program
                ...
                .map_err(|source| NexusError::Resource {
    ...
    fn request_grants(
        &self,
        resources: &[&str],
    ) -> Result<Vec<nexus_kernel::CompiledRequestGrantTemplate>, NexusError> {
        resources
            .iter()
            .map(|resource| {
                let path = Path::parse(resource).map_err(|source| NexusError::Path {
                    resource: (*resource).to_string(),
                    source,
                })?;
                if path.cluster().is_some() {
                    return Err(NexusError::Resource {
                        resource: (*resource).to_string(),
                ...
                let selector = nexus_types::ResourceSelector::parse(&literal).map_err(|source| {
                    NexusError::Resource {
                        resource: (*resource).to_string(),
                        reason: source.to_string(),
                    }
                })?;
                Ok(nexus_kernel::CompiledRequestGrantTemplate {
                    selector,
                    methods,
                })
            })
...
mod tests {
    ...
    #[tokio::test]
    async fn nexus_spawn_actor_runs_under_root() -> anyhow::Result<()> {
        let nx = Nexus::new();
        let spec = ActorSpec {
            name: "worker".into(),
    ...
    #[tokio::test]
    async fn nexus_spawn_actor_with_steps_installs_before_run() -> anyhow::Result<()> {
        let nx = Nexus::new();
        let spec = ActorSpec {
