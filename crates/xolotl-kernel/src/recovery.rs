    ...
    pub fn from_facts(facts: &[Fact]) -> Self {
        ...
        for f in facts {
            ...
            let outcome = match &f.outcome_ref {
                xolotl_types::OutcomeRef::Inline(v) => Outcome::Done(v.clone()),
                xolotl_types::OutcomeRef::External { .. } => {
                    // The full value lives in blob/tensor store; recovery reuses
                    // the reference as the outcome value envelope.
                    Outcome::Done(xolotl_types::Value::Null)
                }
                // A recorded failure replays as the same failure.
                xolotl_types::OutcomeRef::None if !f.decision.is_ok() => Outcome::Fail(
                    xolotl_types::Failure::policy("replay", format!("recorded {:?}", f.decision)),
                ),
                xolotl_types::OutcomeRef::None => continue,
            };
...
pub struct Snapshot {
    ...
    #[serde(default)]
    pub process_state: Option<xolotl_types::Process>,
    ...
    #[serde(default)]
    pub bindings: HashMap<NodeId, xolotl_types::Value>,
}
...
mod tests {
    ...
    use std::sync::Arc;
    use xolotl_state::{StateBackend, StateError, StateResult, TaintedValue};
    use xolotl_types::{
    ...
    impl StateBackend for FailingStateBackend {
        ...
        async fn subscribe(&self, _pattern: &Path) -> StateResult<xolotl_state::StateStream> {
    ...
    async fn recovery_returns_error_when_quarantine_state_write_fails() -> anyhow::Result<()> {
        ...
        let state: xolotl_state::Backend = Arc::new(FailingStateBackend);
