...
pub struct ExecuteParams {
    ...
    pub replay: xolotl_types::ReplayClass,
...
impl DataPlane {
    ...
    /// `replay` is the operation's derived [`xolotl_types::ReplayClass`] used for the Fact
    ...
    async fn execute_inner(
        ...
        let idem_key = if matches!(replay, xolotl_types::ReplayClass::IdempotentEffect) {
            let key = xolotl_types::idempotency::derive_key(op.id, op.acting, &input);
        ...
        if matches!(op.output, OutputMode::Stream) {
            match &result {
                Ok(_) => {
                    if !ctx.emit(Value::StreamEnd(xolotl_types::StreamMarker::Done)) {
                        tracing::debug!(op = ?op.id, "stream end marker receiver closed");
                    }
                }
                Err(e) => {
                    if !ctx.emit(Value::StreamEnd(xolotl_types::StreamMarker::Error {
                        message: e.to_string(),
                    })) {
    ...
    async fn read_idempotent_outcome(
        &self,
        key: &str,
    ) -> xolotl_state::StateResult<Option<Outcome>> {
        ...
        match self.state.read(&path).await? {
            Some(value) => outcome_from_value(value)
                .map(Some)
                .map_err(xolotl_state::StateError::Backend),
            None => Ok(None),
        }
    }
    ...
    fn deny(
        ...
        let fact = self.completed_fact(CompletedFact {
            op,
            resource: resource.unwrap_or_else(|| xolotl_types::ResourceId::new(0)),
            replay,
            now_millis,
            decision: tag,
    ...
    fn completed_fact(&self, input: CompletedFact<'_>) -> Fact {
        ...
        let batch = if batchable {
            xolotl_types::BatchSummary::new(&op.input, Some(&outcome_ref))
        } else {
            None
        };
        Fact {
    ...
    fn attach_stream_sink(
        ...
        let (tx, mut rx) = tokio::sync::mpsc::channel(STREAM_SINK_BUFFER);
    ...
    fn start_async_process(
        ...
        entry.status = xolotl_types::ProcessStatus::Running;
        ...
        let (proc_path, status_path, outcome_path) = match (
            ...
            (proc_path, status_path, outcome_path) => {
                ...
                if processes
                    .mark_finalized_terminal(child, xolotl_types::ProcessStatus::Failed)
                    .is_none()
                {
                    tracing::error!(
                        child = child.get(),
        ...
        let child_op = Operation {
            id: xolotl_types::OperationId::new(child, xolotl_types::NodeId::ROOT, 0),
            process: child,
            acting: op.acting,
            handle: child_handle,
            method: op.method,
        ...
        tokio::spawn(async move {
            ...
            let phase = match actual_status {
                xolotl_types::ProcessStatus::Completed => "completed",
                xolotl_types::ProcessStatus::Failed => "failed",
                xolotl_types::ProcessStatus::Cancelled => "cancelled",
                xolotl_types::ProcessStatus::Created => "created",
                xolotl_types::ProcessStatus::Running => "running",
                xolotl_types::ProcessStatus::Waiting => "waiting",
                xolotl_types::ProcessStatus::Suspended => "suspended",
                xolotl_types::ProcessStatus::Finalizing => "finalizing",
            };
...
fn stream_path(op: &Operation) -> Result<Path, xolotl_types::PathError> {
    Path::try_new("state")?
        .try_push("stream")?
...
fn collect_stream_path() -> Result<Path, xolotl_types::PathError> {
    Path::try_new("state")?
        .try_push("stream")?
        .try_push("collect")
}
...
fn async_proc_path(process: xolotl_types::ProcessId) -> Result<Path, xolotl_types::PathError> {
    Path::try_new("proc")?
        .try_push("async")?
        .try_push_literal(process.get().to_string())
}

fn async_status_path(process: xolotl_types::ProcessId) -> Result<Path, xolotl_types::PathError> {
    Path::try_new("state")?
        .try_push("kernel")?
        .try_push("async")?
        .try_push_literal(process.get().to_string())?
...
fn async_outcome_path(process: xolotl_types::ProcessId) -> Result<Path, xolotl_types::PathError> {
    Path::try_new("state")?
        .try_push("kernel")?
        .try_push("async")?
        .try_push_literal(process.get().to_string())?
...
struct AsyncRefMeta {
    parent: xolotl_types::ProcessId,
    child: xolotl_types::ProcessId,
    resource: xolotl_types::ResourceId,
    method: xolotl_types::MethodId,
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl xolotl_state::StateBackend for FailingAppendState {
        async fn read_tainted(
            &self,
            _path: &Path,
        ) -> StateResult<Option<xolotl_state::TaintedValue>> {
            Ok(None)
        }
        ...
        async fn read_prefix_tainted(
            &self,
            _prefix: &Path,
        ) -> StateResult<Vec<(Path, xolotl_state::TaintedValue)>> {
            Ok(Vec::new())
        }
        ...
        let stream = state
            ...
            stream
                == Some(Value::List(vec![
                    ...
                    Value::StreamEnd(xolotl_types::StreamMarker::Done),
                ])),
    ...
    async fn stream_append_failure_returns_driver_error() -> anyhow::Result<()> {
        ...
        let (facts, _) = FactSink::in_memory();
        let state: xolotl_state::Backend = Arc::new(FailingAppendState);
    ...
    async fn sink_only_suppresses_response_body() -> anyhow::Result<()> {
        ...
        let fact = facts.first().context("missing sink-only fact")?;
            ...
            matches!(
                fact.outcome_ref,
                xolotl_types::OutcomeRef::Inline(Value::Null)
            ),
