...
impl ProcessTable {
    ...
    pub(crate) fn take_finalizers(&self, id: ProcessId) -> Vec<xolotl_graph::DoNode> {
        let mut inner = self.inner.write();
...
mod tests {
    ...
    fn finalizers_run_in_reverse() -> anyhow::Result<()> {
        ...
        entry
            .on_finalize
            .push(xolotl_graph::DoNode::pure(xolotl_types::Value::Int(1)));
        entry
            .on_finalize
            .push(xolotl_graph::DoNode::pure(xolotl_types::Value::Int(2)));
        t.insert(entry);
        let fs = t.take_finalizers(p);
        // Added 1 then 2; reverse order runs 2 then 1.
        let first = fs.first().context("missing first finalizer")?;
        ensure!(
            *first == xolotl_graph::DoNode::pure(xolotl_types::Value::Int(2)),
            "first finalizer mismatch"
        );
        let second = fs.get(1).context("missing second finalizer")?;
        ensure!(
            *second == xolotl_graph::DoNode::pure(xolotl_types::Value::Int(1)),
