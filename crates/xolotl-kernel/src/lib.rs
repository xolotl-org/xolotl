...
//! `xolotl-kernel` - the Xolotl runtime kernel.
//!
//! The control plane is a compiler: `open()` compiles a Grant against a
//! Resource into a [`Handle`]. The data plane is a VM that
//! executes [`Operation`](xolotl_types::Operation)s through that handle's frozen
//! [`DriverPlan`], recording a [`Fact`](xolotl_types::Fact).
//! The [`Executor`] advances an
//! [`ExecutionGraph`](xolotl_graph::ExecutionGraph) cursor.
//!
//! Public modules:
//!
