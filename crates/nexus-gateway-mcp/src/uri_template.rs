...
mod tests {
    ...
    fn templates_match_and_expose_variables() -> anyhow::Result<()> {
            ...
            mcp_uri_template_matches("nexus://files/{path}", "nexus://files/a/b")?,
            ...
            mcp_uri_template_variable_names("nexus://files/{+path}{?rev}")?
                .into_iter()
                .collect::<Vec<_>>()
            ...
            !mcp_uri_template_matches("nexus://files/{path}.md", "nexus://files/a.txt")?,
            "template matched a mismatched extension"
        );
        ensure!(
            validate_mcp_uri_template("nexus://bad/{").is_err(),
