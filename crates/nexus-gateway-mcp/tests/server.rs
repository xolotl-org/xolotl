...
use nexus_kernel::{Bootstrap, EchoDriver, FnDriver};
use nexus_types::{Outcome, OutcomeRef, Value};
use serde_json::json;
...
fn register_echo(boot: &Bootstrap, effect: &str) -> anyhow::Result<nexus_types::ResourceName> {
...
fn register_fixed(
    ...
    effect: &str,
    value: Value,
) -> anyhow::Result<nexus_types::ResourceName> {
    must(boot.register_effect(
        effect,
        &[nexus_kernel::MethodSpec::new(
            "invoke",
...
fn echo_profile(
    target: nexus_types::ResourceName,
...
async fn descriptors_are_authenticated_and_kind_filtered() -> anyhow::Result<()> {
    ...
    .with_publication(
        mcp_tool_publication("echo", "echo")
            ...
            .with_metadata(Value::Map(BTreeMap::from([(
                "owner".into(),
                Value::from("nexus-test"),
            )]))),
    )
    .with_publication(
        mcp_resource_publication("echo-resource", "nexus://echo/value", "echo")
    ...
    assert_eq!(resources[0].uri, "nexus://echo/value");
...
async fn jsonrpc_lists_and_calls_tools() -> anyhow::Result<()> {
        ...
        mcp_tool_publication("echo", "echo")
            ...
            .with_property(
                ...
                Value::List(vec![Value::Map(BTreeMap::from([(
                    "src".into(),
                    Value::from("nexus://icons/echo.png"),
                )]))]),
            )
            .with_metadata(Value::Map(BTreeMap::from([(
                "owner".into(),
    ...
    assert_eq!(tools[0]["icons"][0]["src"], "nexus://icons/echo.png");
    assert_eq!(tools[0]["_meta"]["owner"], "nexus-test");
        ...
        (
            ...
            Value::List(vec![
                ...
                Value::Map(BTreeMap::from([
                    ...
                    ("uri".into(), Value::from("nexus://docs/readme")),
                ...
                Value::Map(BTreeMap::from([
                    ...
                    (
                        "resource".into(),
                        Value::Map(BTreeMap::from([
                            ("uri".into(), Value::from("nexus://docs/embed")),
...
async fn jsonrpc_lists_and_reads_resources() -> anyhow::Result<()> {
    ...
    .with_publication(
        mcp_resource_publication("readme", "nexus://docs/readme", "resource")
            .with_title("Readme")
    ...
    assert_eq!(resources[0]["uri"], "nexus://docs/readme");
...
async fn jsonrpc_reads_binary_resource_content() -> anyhow::Result<()> {
    ...
    .with_publication(
        mcp_resource_publication("binary", "nexus://blob/binary", "resource")
...
async fn jsonrpc_lists_templates_and_routes_template_reads() -> anyhow::Result<()> {
    ...
    .with_publication(
        mcp_resource_template_publication("file", "nexus://files/{path}", "template")
            .with_property("mimeType", Value::from("text/plain")),
    )
    .with_principal_surface_binding(GatewayPrincipalSurfaceBinding::allow(
        "alice",
    ...
    assert_eq!(templates[0]["uriTemplate"], "nexus://files/{path}");
    ...
    let read = mcp
        .handle_jsonrpc_value(
            ...
            json!({
                ...
                "id":2,
                "method":"resources/read",
                "params":{"uri":"nexus://files/readme.md"}
            }),
        )
        .await;
    assert!(read.error.is_none());
    ...
    assert_eq!(
        text,
        "{\"uri\":\"nexus://files/readme.md\",\"uriTemplate\":\"nexus://files/{path}\"}"
    );
    Ok(())
}
