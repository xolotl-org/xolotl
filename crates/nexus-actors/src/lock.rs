...
/// `invoke`.
pub const LOCK_METHODS: &[MethodSpec] = &[
        ...
        match method.get() {
            ...
            0 => {
                ...
                match self.state.write_cas(&path, None, holder).await {
                    ...
                    Err(_) => Ok(Outcome::Done(Value::Bool(false))), // already held
                }
            }
...
mod tests {
    ...
    async fn acquire_is_exclusive_until_released() {
        ...
        let a = d
            ...
            .await
            .unwrap();
        assert_eq!(a, Outcome::Done(Value::Bool(true)));
        ...
        let b = d
            ...
            .await
            .unwrap();
        assert_eq!(b, Outcome::Done(Value::Bool(false)));
        ...
        let c = d
            ...
            .await
            .unwrap();
        assert_eq!(c, Outcome::Done(Value::Bool(true)));
    }

    #[tokio::test]
    async fn illegal_lock_name_is_a_driver_error_not_a_panic() {
        // A name with an illegal path char (space) used to panic via expect();
        // it must now surface as a DriverError (caller error), never crash.
        let state: Backend = Arc::new(InMemoryBackend::new());
        let d = LockDriver::new(state);
        let ctx = DriverContext::new(IdentityRef::ROOT, ProcessId::new(1));
        let r = d
            ...
            .await;
        assert!(r.is_err(), "illegal lock name must be a DriverError");
    }
}
