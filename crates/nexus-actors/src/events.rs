...
/// `invoke`.
pub const EVENTS_METHODS: &[MethodSpec] = &[
    ...
    async fn stream_topic(
        ...
        while delivered < limit {
            match rx.recv().await {
                ...
                Err(_) => break,   // lagged/closed: end the stream cleanly
            }
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn publish_appends_to_topic() {
        ...
        .unwrap();
        let stored = state
            .read(&EventBusDriver::topic_path("alerts").unwrap())
            .await
            .unwrap();
        assert_eq!(stored, Some(Value::List(vec![Value::Str("fire".into())])));
    }

    #[tokio::test]
    async fn subscribe_unary_returns_topic_path() {
        let state: Backend = Arc::new(InMemoryBackend::new());
        let d = EventBusDriver::new(state.clone());
        let mut m = BTreeMap::new();
        ...
        assert_eq!(
            out,
            Outcome::Done(Value::Str("state://events/alerts".into()))
        );
    }

    #[tokio::test]
    async fn published_event_reaches_streaming_subscriber() {
        let state: Backend = Arc::new(InMemoryBackend::new());
        let d = Arc::new(EventBusDriver::new(state.clone()));

        // A streaming subscriber: capture emitted chunks through the stream sink.
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel();
        let sink_path = Path::parse("state://stream/sub").unwrap();
        let sctx =
            DriverContext::new(IdentityRef::ROOT, ProcessId::new(2)).with_stream(sink_path, tx);

        // Bound the subscriber so it terminates after the events we publish.
        ...
        assert_eq!(
            out,
            Outcome::Done(Value::Int(2)),
            "subscriber saw both events"
        );
        assert_eq!(rx.recv().await, Some(Value::Str("hello".into())));
        assert_eq!(rx.recv().await, Some(Value::Str("world".into())));
    }
}
