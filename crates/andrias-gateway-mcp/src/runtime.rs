...
use andrias_kernel::GatewayAudit;
use andrias_types::{Outcome, Value};
use serde_json::json;
use std::collections::BTreeMap;
use std::sync::Arc;

/// Transport-neutral MCP server adapter over a shared Andrias Gateway.
pub struct McpGateway<G: Gateway> {
    gateway: Arc<G>,
}

impl<G: Gateway> McpGateway<G> {
    /// Create an adapter over an existing authenticated Andrias gateway.
