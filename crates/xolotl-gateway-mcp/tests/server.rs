...
fn register_echo(boot: &Bootstrap, effect: &str) -> anyhow::Result<xolotl_types::ResourceName> {
...
fn register_fixed(
    ...
    effect: &str,
    value: Value,
) -> anyhow::Result<xolotl_types::ResourceName> {
    must(boot.register_effect(
        effect,
        &[xolotl_kernel::MethodSpec::new(
            "invoke",
...
fn echo_profile(
    target: xolotl_types::ResourceName,
...
async fn descriptors_are_authenticated_and_kind_filtered() -> anyhow::Result<()> {
    ...
    .with_publication(
        mcp_resource_publication("echo-resource", "xolotl://echo/value", "echo")
    ...
    assert_eq!(resources[0].uri, "xolotl://echo/value");
...
async fn jsonrpc_lists_and_calls_tools() -> anyhow::Result<()> {
    ...
    assert_eq!(tools[0]["icons"][0]["src"], "xolotl://icons/echo.png");
    assert_eq!(tools[0]["_meta"]["owner"], "xolotl-test");
...
async fn jsonrpc_lists_and_reads_resources() -> anyhow::Result<()> {
    ...
    .with_publication(
        mcp_resource_publication("readme", "xolotl://docs/readme", "resource")
            .with_title("Readme")
    ...
    assert_eq!(resources[0]["uri"], "xolotl://docs/readme");
...
async fn jsonrpc_reads_binary_resource_content() -> anyhow::Result<()> {
    ...
    .with_publication(
        mcp_resource_publication("binary", "xolotl://blob/binary", "resource")
...
async fn jsonrpc_lists_templates_and_routes_template_reads() -> anyhow::Result<()> {
    ...
    assert_eq!(templates[0]["uriTemplate"], "xolotl://files/{path}");
    ...
    let read = mcp
        .handle_jsonrpc_value(
            ...
            json!({
                ...
                "id":2,
                "method":"resources/read",
                "params":{"uri":"xolotl://files/readme.md"}
            }),
        )
        .await;
    assert!(read.error.is_none());
    ...
    assert_eq!(
        text,
        "{\"uri\":\"xolotl://files/readme.md\",\"uriTemplate\":\"xolotl://files/{path}\"}"
    );
    Ok(())
}
