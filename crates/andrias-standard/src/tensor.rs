...
use andrias_types::{DType, MethodId, Outcome, OutputMode, Path, Purity, TensorRef, Value};
        ...
        match method.get() {
            ...
            0 => {
                ...
                let blob = crate::blob::write_blob_bytes(
                    &self.state,
                    bytes,
                    Some("application/x-andrias-tensor".into()),
                )
                .await?;
...
fn numeric_f64(v: &Value) -> Result<f64, DriverError> {
    match v {
        Value::Float(andrias_types::FloatBits(f)) => Ok(*f),
...
mod tests {
    ...
    use andrias_types::{FloatBits, IdentityRef, ProcessId};
