...
use andrias_types::{MethodId, Outcome, OutputMode, ProcessId, Purity, Value};
...
pub(crate) struct KernelInspectDriver {
    processes: andrias_kernel::ProcessTable,
    facts: andrias_kernel::FactSink,
}
...
fn project_fact(f: &andrias_types::Fact) -> Value {
...
mod tests {
    ...
    use andrias_types::{IdentityRef, NodeId, OperationId};
    ...
    async fn inspect_can_include_recent_facts() -> Result<()> {
        let boot = Bootstrap::in_memory();
        boot.record_gateway_audit(andrias_kernel::GatewayAudit {
            event: "test",
            username: None,
            source_addr: None,
            outcome: "ok",
