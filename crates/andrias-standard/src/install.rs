...
use andrias_kernel::{Bootstrap, BootstrapError, Driver, DriverContext, DriverError, MethodSpec};
use andrias_types::{
...
use andrias_types::{EffectCapability, Metadata, Purity};
...
pub enum InstallError {
    ...
    Path {
        ...
        #[source]
        /// Parser error.
        source: andrias_types::PathError,
    },
...
impl From<andrias_state::StateError> for InstallError {
...
fn default_inference_driver(state: andrias_state::Backend) -> InferenceDriver {
...
fn default_inference_driver(_: andrias_state::Backend) -> InferenceDriver {
...
fn install_core_standard(
    ...
    if config.modules.contains(StandardModule::Fact) {
        boot.register_subtree_resource_at(
            "state://fact",
            "read://state/fact/**",
            andrias_types::InterfaceFamily::Sequence,
            FACT_METHODS,
    ...
    if config.modules.contains(StandardModule::State) {
        boot.register_subtree_resource(
            "state",
            andrias_types::InterfaceFamily::Value,
...
fn ensure_declared_effect_owner(
    ...
    let name = andrias_types::ResourceName::new(parse_path(effect_path)?);
    let resource_id = match boot.kernel.registry.resolve_resource(&name) {
        Ok(resource_id) => resource_id,
        Err(andrias_kernel::ResolveError::NoSuchResource(_)) => return Ok(()),
    };
...
fn register_single_effect(
    ...
    methods: &[MethodSpec],
    driver: Arc<dyn Driver>,
) -> Result<andrias_types::ResourceName, InstallError> {
...
mod tests {
    ...
    fn resource_name(path: &str) -> anyhow::Result<andrias_types::ResourceName> {
        andrias_types::Path::parse(path)
            .map(andrias_types::ResourceName::new)
    ...
    async fn state_write_persists_taint() -> anyhow::Result<()> {
        ...
        let entry_taint = andrias_types::TaintSet::of(andrias_types::TaintSource::Fetched {
    ...
    async fn fact_read_side_is_state_projection_not_effect_alias() -> anyhow::Result<()> {
        ...
        let err = boot
            ...
            matches!(err, andrias_kernel::OpenError::ReservedPath(_)),
    ...
    fn external_observation_methods_are_registered_as_observation_replay() -> anyhow::Result<()> {
        ...
        assert_method_replay(
            ...
            "state://fact/1",
            "read",
            andrias_types::ReplayClass::Observation,
        )?;
        assert_method_replay(
            &boot,
            "effect://time/now",
        ...
        assert_method_replay(
            ...
            "effect://rank/fuse",
            "invoke",
            andrias_types::ReplayClass::Deterministic,
        )?;
        Ok(())
    }
    ...
    fn assert_method_replay(
        ...
        path: &str,
        method: &str,
        expected: andrias_types::ReplayClass,
    ) -> anyhow::Result<()> {
        let name = resource_name(path)?;
        let rid = boot
            .kernel
