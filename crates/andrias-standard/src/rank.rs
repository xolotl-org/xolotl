...
use andrias_types::{FloatBits, MethodId, Outcome, OutputMode, Purity, Value};
...
impl RankerDriver {
    ...
    pub(crate) fn with_state(mut self, state: andrias_state::Backend) -> Self {
    ...
    async fn resolve_weights(
        ...
        if let Some(state) = &self.state {
            let path =
                andrias_types::Path::parse("state://kernel/rank/weights").map_err(|error| {
...
mod tests {
    ...
    fn path(value: &str) -> Result<andrias_types::Path> {
        andrias_types::Path::parse(value).with_context(|| format!("parse {value}"))
    }
