...
use andrias_types::{BlobRef, DType, TensorRef, Value};
...
fn vector_value(
    ...
    for item in vector {
        ...
        values.push(Value::Float(andrias_types::FloatBits(n)));
    }
    let digest = hash.finalize();
    let blob = BlobRef {
        hash: digest.to_hex().to_string(),
        size: (vector.len() * std::mem::size_of::<f32>()) as u64,
        mime: Some("application/x-andrias-http-inference-embedding".into()),
    };
