...
use andrias_types::{MethodId, Outcome, OutputMode, Path, ProcSpec, Purity, Transport, Value};
...
impl ProcDriver {
    ...
    fn status_value(
        ...
        m.insert(
            "started_at".into(),
            Value::Int(andrias_kernel::now_millis()),
        );
...
fn optional_stored_pid(m: &BTreeMap<String, Value>) -> Result<Option<u32>, DriverError> {
    match m.get("pid") {
        None => Ok(None),
        Some(Value::Int(pid)) if *pid > 0 => u32::try_from(*pid)
            .map(Some)
            .map_err(|_| DriverError::Other("malformed proc status: pid is out of range".into())),
...
#[cfg(test)]
fn supervise(
    policy: &andrias_types::external::RestartPolicy,
    failures_in_window: u32,
    attempt: u32,
) -> SuperviseDecision {
    use andrias_types::external::{Backoff, RestartPolicy};
    match policy {
...
mod tests {
    ...
    fn stdio_spec_input(id: &str, command: Vec<&str>) -> anyhow::Result<Value> {
        ...
        let spec = ProcSpec {
            ...
            env: BTreeMap::new(),
            cwd: None,
            restart: andrias_types::RestartPolicy::Never,
        };
    ...
    async fn heartbeat_writes_health() -> anyhow::Result<()> {
        ...
        d.call(MethodId::new(4), Value::Map(m), OutputMode::Unary, &ctx())
            .await?;
        let health_path = andrias_types::Path::parse("state://kernel/procs/ext-h/health")
    ...
    async fn heartbeat_rejects_malformed_status_without_writing_health() -> anyhow::Result<()> {
        ...
        let health_path = andrias_types::Path::parse("state://kernel/procs/ext-corrupt-hb/health")
            .context("parse health path")?;
        ensure!(
    ...
    fn supervise_on_failure_gives_up_past_budget() {
        use andrias_types::external::RestartPolicy;
