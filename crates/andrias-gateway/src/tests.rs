...
use andrias_proto::andrias::v1::external as external_pb;
use andrias_types::external::{Role as ExternalRole, SessionContext as ExternalSessionContext};
...
impl andrias_kernel::Driver for BlockingCountingDriver {
    async fn call(
        &self,
        _method: andrias_types::MethodId,
        input: Value,
        _output: OutputMode,
        _ctx: &andrias_kernel::DriverContext,
    ) -> Result<Outcome, andrias_kernel::DriverError> {
...
#[async_trait::async_trait]
impl andrias_kernel::Driver for FailingDriver {
    async fn call(
        &self,
        _method: andrias_types::MethodId,
        _input: Value,
...
fn restricted_anchor(boot: &Bootstrap, selector: &str) -> anyhow::Result<ProcessId> {
    boot.spawn_request_process_under_with_request_grants(
        ...
        &[andrias_kernel::RequestGrantTemplate {
            literal: selector,
            methods: andrias_types::MethodBitmap::ALL,
        }],
    )
    .map_err(Into::into)
}
...
async fn submit_runs_direct_input_surface() -> anyhow::Result<()> {
    ...
    let name = boot.register_effect(
        ...
        Arc::new(andrias_kernel::EchoDriver),
    )?;
...
async fn surface_output_schema_does_not_validate_sink_only_delivery() -> anyhow::Result<()> {
    ...
    let name = boot.register_effect(
        ...
        &[andrias_kernel::MethodSpec::new(
            "invoke",
            andrias_types::Purity::Pure,
            andrias_kernel::MethodSpec::SINK_ASYNC,
        )],
        Arc::new(andrias_kernel::EchoDriver),
    )?;
    let profile = bind_alice_to_echo(
...
async fn direct_input_large_ref_requires_provenance() -> anyhow::Result<()> {
    ...
    let session = gw
        .authenticate(PresentedCredential::bearer(TEST_TOKEN))
        .await?;
    let blob = Value::Blob(andrias_types::BlobRef {
        hash: "abc".into(),
        size: 3,
