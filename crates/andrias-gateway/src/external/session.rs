...
use andrias_kernel::{CheckCtx, PolicyDecision, PolicySnapshot};
use andrias_state::{Backend, StateError};
use andrias_types::ErrorInfo;
...
use andrias_types::{IdentityRef, Path, ResourceId, TaintSet, TaintSource, Value};
    ...
    while let Some(value) = stack.pop() {
        match value {
            ...
            Value::StreamEnd(marker) => match marker {
                andrias_types::StreamMarker::Done => total = total.saturating_add(1),
...
async fn append_source_event(
    req: &SourceIngest<'_>,
    emits: &andrias_types::external::EventSource,
    event: &InboundEvent,
    taint: TaintSet,
) -> Result<(), SourceIngestError> {
...
async fn admit_source_ingest<'a>(
    ...
    event: &InboundEvent,
) -> Result<&'a andrias_types::EventSource, SourceIngestError> {
    req.session
...
/// Validate a Andrias value against the external schema subset.
...
mod tests {
    use super::*;
    use andrias_kernel::{CompiledCheck, PolicyDecision};
    use andrias_state::InMemoryBackend;
    use andrias_types::external::{
        EventSource, OverflowPolicy, Role, SourceRateLimit, StreamCapacity,
    };
    use andrias_types::{FloatBits, MethodId, Purity, TaintSource};
    ...
    fn source_event_sink() -> anyhow::Result<Path> {
        andrias_types::sandboxed_source_event_sink_path("inst-1", "source")
