...
//! `andrias-gateway` - the shared Gateway runtime.
//!
//! A Gateway verifies transport credentials, maps them to Andrias identities, and
...
use andrias_graph::{DoNode, OperationTemplate, WaitSpec};
use andrias_kernel::{
...
impl GatewayRuntime {
    ...
    fn compile_profile(
        ...
        if let Some(anchor) = profile.authority_anchor {
            ...
            let now_millis = andrias_kernel::now_millis();
...
async fn reserve_submission_idempotency_if_present(
    ...
    match state.write_cas(&path, None, pending_record.clone()).await {
        ...
        Err(andrias_state::StateError::CasFailed { .. }) => {
            let current = state
                .read(&path)
                .await
...
async fn commit_submission_idempotency_outcome(
    ...
    state
        ...
        .await
        .map_err(|e| match e {
            andrias_state::StateError::CasFailed { .. } => {
...
fn submission_hash(submission: &GatewaySubmission) -> Result<String, GatewayError> {
    ...
    update_hash_str(&mut hasher, "andrias-gateway-submission-v1");
...
fn effective_idempotency_hash(
    ...
    let mut hasher = blake3::Hasher::new();
    update_hash_str(&mut hasher, "andrias-gateway-idempotency-v1");
...
fn hash_idempotency_material(kind: &str, material: &str) -> String {
    let mut hasher = blake3::Hasher::new();
    update_hash_str(&mut hasher, "andrias-gateway-idempotency-material-v1");
...
fn modality_allows_value(modality: GatewayModality, value: &Value) -> bool {
    match (modality, value) {
        ...
        (GatewayModality::AudioFrame, Value::Frame(frame)) => {
            frame.kind == andrias_types::FrameKind::Audio
        }
        (GatewayModality::VideoFrame, Value::Frame(frame)) => {
            frame.kind == andrias_types::FrameKind::Video
        }
        (GatewayModality::PoseFrame, Value::Frame(frame)) => {
            frame.kind == andrias_types::FrameKind::Pose
        }
        (GatewayModality::SensorFrame, Value::Frame(frame)) => {
            frame.kind == andrias_types::FrameKind::Sensor
        }
        _ => false,
    }
}
...
fn state_path(segments: &[&str]) -> Result<Path, andrias_types::PathError> {
    ...
    while let Some(value) = stack.pop() {
        match value {
            ...
            Value::StreamEnd(marker) => match marker {
                andrias_types::StreamMarker::Done => {
                    add_literal_bytes_with_label(inspection, 1, limits, label)?
                }
                andrias_types::StreamMarker::Error { message } => {
