...
use andrias_types::{ConstraintSet, Grant, HandleId, IdentityRef, ProcessId, ResourceId, Rights};
use std::sync::Arc;
use thiserror::Error;
...
pub struct OpenRequest {
    ...
    pub requested_path: Option<andrias_types::Path>,
...
pub fn open_resource_with_attached(
    ...
    if andrias_types::is_kernel_reserved(&resource_name)
        || andrias_types::is_vault_reserved(&resource_name)
        || (andrias_types::is_fact_reserved(&resource_name) && !is_fact_projection_resource)
    {
...
mod tests {
    ...
    fn setup_resource(reg: &Registry, path: &str) -> anyhow::Result<ResourceId> {
        ...
        reg.register_interface(Interface {
            ...
            methods: vec![Method {
                id: andrias_types::MethodId::new(100),
                name: "invoke".into(),
        ...
        reg.register_driver(DriverDescriptor {
            ...
            transport: andrias_types::Transport::InProcess,
        ...
        reg.register_binding(Binding {
            ...
            driver: andrias_types::DriverRef {
                id: driver_id,
                name: "echo".into(),
            },
            endpoint: None,
    ...
    fn setup_remote_resource(
        ...
        reg.register_interface(Interface {
            ...
            family: InterfaceFamily::Callable,
            methods: vec![Method {
                id: andrias_types::MethodId::new(0),
                name: "invoke".into(),
                input: SchemaId::new(0),
                output: SchemaId::new(0),
                modality: ModalitySet::TEXT,
        ...
        reg.register_driver(DriverDescriptor {
            ...
            implements: InterfaceSet::new(vec![iface_id]),
            transport: andrias_types::Transport::Grpc {
    ...
    impl RemoteEndpoint for TestEndpoint {
        async fn invoke(
            ...
            Ok(InvokeResult {
                ...
                outcome: Ok(andrias_types::Value::Str("remote".into())),
            })
        }
    }
    ...
    fn open_unconditional_when_no_constraints() -> anyhow::Result<()> {
        ...
        let h = handles.get(id).context("opened handle did not resolve")?;
            ...
            h.driver_plan.supports(andrias_types::MethodId::new(100)),
    ...
    async fn endpoint_binding_compiles_to_remote_driver_plan() -> anyhow::Result<()> {
        ...
        let h = handles.get(id).context("opened handle did not resolve")?;
            ...
            h.driver_plan.supports(andrias_types::MethodId::new(0)),
            "driver plan should support method 0"
        );

        let ctx = crate::DriverContext::new(IdentityRef::ROOT, ProcessId::new(1))
            .with_operation_id(andrias_types::OperationId::new(
                ProcessId::new(1),
                andrias_types::NodeId::new(4),
                0,
            ));
        let out = h
            .driver_plan
            .call(
                andrias_types::MethodId::new(0),
                andrias_types::Value::Str("q".into()),
                andrias_types::OutputMode::Unary,
                &ctx,
            )
            .await
            .context("remote driver call failed")?;
        ensure!(
            out == andrias_types::Outcome::Done(andrias_types::Value::Str("remote".into())),
    ...
    fn open_conditional_when_constraints_present() -> anyhow::Result<()> {
        ...
        reg.register_grant(Grant {
            ...
            constraints: ConstraintSet {
                predicates: vec![andrias_types::cap::Predicate::parse("account=alice")?],
            },
            expires: Expiry::Never,
        });
        let mut handles = HandleTable::new();
    ...
    fn registered_policy_adds_residual_check() -> anyhow::Result<()> {
        ...
        reg.register_policy(Arc::new(CapabilityPolicy {
            pattern: andrias_types::Capability::parse("perform://effect/x/**")?,
            constraints: ConstraintSet {
                predicates: vec![andrias_types::cap::Predicate::parse("account=alice")?],
            },
        }));
