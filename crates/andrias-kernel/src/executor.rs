...
pub struct Executor {
    ...
    state: Option<andrias_state::Backend>,
...
struct MethodMeta {
    ...
    method_index: u32,
    method_id: andrias_types::MethodId,
    replay: ReplayClass,
    supports: andrias_types::OutputModeSet,
...
struct Env {
    ...
    binding_taint: HashMap<NodeId, andrias_types::TaintSet>,
    acting: IdentityRef,
    /// Provenance of the value flowing into the current node.
    taint: andrias_types::TaintSet,
}
...
impl Env {
    ...
    fn with_taint(&self, taint: andrias_types::TaintSet) -> Self {
...
impl Executor {
    ...
    pub fn with_state(mut self, state: andrias_state::Backend) -> Self {
    ...
    fn is_cancelled(&self) -> bool {
        match &self.processes {
            Some(p) => match p.status(self.process) {
                Some(andrias_types::ProcessStatus::Cancelled) => true,
                Some(andrias_types::ProcessStatus::Finalizing) => !self.finalizer_mode,
                _ => false,
            },
            None => false,
        }
    ...
    pub fn bind_handle(&self, name: ResourceName, handle: andrias_types::HandleId) {
    ...
    pub async fn eval(&self, program: &DoNode) -> Outcome {
        let graph = match compile_do(program) {
            Ok(g) => g,
            Err(e) => {
                return Outcome::Fail(andrias_types::Failure::policy(
                    "executor",
    ...
    pub async fn eval_graph(&self, graph: &ExecutionGraph) -> Outcome {
        self.eval_graph_tainted(graph, andrias_types::TaintSet::pristine())
            .await
    }
    ...
    pub async fn eval_tainted(
        &self,
        program: &DoNode,
        entry_taint: andrias_types::TaintSet,
    ) -> Outcome {
        let graph = match compile_do(program) {
            Ok(g) => g,
            Err(e) => {
            ...
            match &node.kind {
                NodeKind::Pure(v) => {
                    ...
                    let (out, taint) = match self.resolve_use(graph, id, env) {
                        ...
                        Ok(None) => (v.clone(), andrias_types::TaintSet::author()),
                    ...
                    if self.is_cancelled() {
                        return self
                            .continue_with(
                                graph,
                                id,
                                Outcome::Fail(andrias_types::Failure::Cancelled),
                                env,
                                depth,
                                next_base,
                            )
                ...
                NodeKind::Join(kind) => {
                    ...
                    let out = match (arms.first().copied(), arms.get(1).copied()) {
                        ...
                        _ => Outcome::Fail(andrias_types::Failure::policy(
                            "executor",
                ...
                NodeKind::Acting(path) => {
                    ...
                    if !self.authorize_act_as(path) {
                        let out = Outcome::Fail(andrias_types::Failure::policy(
                            "act-as",
                            format!(
    ...
    fn resolve_use(
        ...
        id: NodeId,
        env: &Env,
    ) -> Result<Option<(Value, andrias_types::TaintSet)>, andrias_types::Failure> {
        ...
        let Some(v) = env.bindings.get(&producer).cloned() else {
            return Err(andrias_types::Failure::policy(
                "executor",
    ...
    async fn run_step(
        ...
        let sub = match self.steps.get(self.process, &sref.name) {
            Some(f) => {
                match std::panic::catch_unwind(AssertUnwindSafe(|| f(piped, sref.arg.clone()))) {
                    Ok(sub) => sub,
                    Err(payload) => {
                        return Outcome::Fail(andrias_types::Failure::HandlerError {
                            kind: "panic".into(),
        ...
        let subgraph = match andrias_graph::compile_do_at(&sub, base) {
            Ok(g) => g,
            Err(e) => {
                return Outcome::Fail(andrias_types::Failure::policy(
                    "executor",
    ...
    async fn run_wait(&self, spec: &WaitSpec) -> Outcome {
        match spec {
            ...
            WaitSpec::Signal(path) => match &self.state {
                ...
                None => Outcome::Fail(andrias_types::Failure::policy(
                    "executor",
    ...
    async fn run_operation(
        ...
        position: NodeId,
        record: bool,
    ) -> (Outcome, andrias_types::TaintSet) {
        ...
        if is_outbound(&tmpl.target) && op_taint.has_protected() {
            return (
                Outcome::Fail(andrias_types::Failure::PolicyViolation {
                    policy: "taint".into(),
                    detail: format!(
        ...
        let Some(meta) = self.resolve_meta(&tmpl.target, &tmpl.method) else {
            return (
                Outcome::Fail(andrias_types::Failure::NoHandler {
        ...
        if !tmpl.output.is_supported_by(supports) {
            return (
                Outcome::Fail(andrias_types::Failure::InvalidInput {
                    reason: format!(
        ...
        let handle = match self.handle_for_or_open(
            ...
            Err(error) => {
                return (
                    Outcome::Fail(andrias_types::Failure::policy(
                        "open",
                        format!(
            ...
            && let Some(procs) = &self.processes
        {
            ...
            if let Err(dim) = procs.reserve(self.process, est_usd, est_tokens) {
                let out = self.data_plane.record_pre_dispatch_denial(
                    ...
                    andrias_types::Failure::BudgetExhausted { dim },
                );
    ...
    fn authorize_act_as(&self, identity: &andrias_types::Path) -> bool {
        let now = now_millis();
        ...
        grants.into_iter().any(|g| {
            !g.expires.is_expired(now)
                && g.rights.flags.contains(andrias_types::RightFlags::DELEGATE)
    ...
    fn handle_for_or_open(
        ...
        resource_id: andrias_types::ResourceId,
        method_index: u32,
    ) -> Result<andrias_types::HandleId, crate::open::OpenError> {
...
fn is_process_local_state_path(path: &andrias_types::Path, process: ProcessId) -> bool {
...
fn estimate_cost(
    ...
    match (batchable, input) {
        (true, Value::List(items)) => {
            ...
            let variable = andrias_types::CostModel {
                flat_micro_usd: 0,
                ..*cost
            }
...
/// `Acting` blocks.
pub fn intern_identity(path: &andrias_types::Path) -> IdentityRef {
...
fn intrinsic_source(target: &ResourceName) -> Option<andrias_types::TaintSource> {
    ...
    match scheme {
        ...
        "effect" if first == "inference" || first == "deliberation" => {
            Some(andrias_types::TaintSource::ModelOutput)
        }
        ...
        "effect" if first == "fetch" => {
            ...
            Some(andrias_types::TaintSource::Fetched { host: host.into() })
        }
        // Reads from a protected state prefix carry the secret's lineage.
        "state" if andrias_types::is_vault_reserved(path) => {
            Some(andrias_types::TaintSource::Protected { path: path.clone() })
        }
        _ => None,
    }
}
...
mod tests {
    ...
    impl Driver for TestStateReadDriver {
        async fn call(
            ...
            method: MethodId,
            _input: Value,
            _output: andrias_types::OutputMode,
            ctx: &DriverContext,
    ...
    async fn acting_denied_without_delegate_grant() -> anyhow::Result<()> {
        ...
        let prog = DoNode::acting(
            andrias_types::Path::parse("process/bob")?,
            DoNode::pure(Value::Int(1)),
        );
        match ex.eval(&prog).await {
            Outcome::Fail(andrias_types::Failure::PolicyViolation { policy, .. }) => {
    ...
    async fn acting_allowed_with_delegate_grant() -> anyhow::Result<()> {
        use andrias_types::{Expiry, Grant, MethodBitmap, ResourceSelector, RightFlags, Rights};
        let (facts, _) = FactSink::in_memory();
        let dp = DataPlane::new(
            Arc::new(RwLock::new(HandleTable::new())),
            facts,
        ...
        reg.register_grant(Grant {
            ...
            constraints: andrias_types::ConstraintSet::empty(),
    ...
    async fn cancelled_process_short_circuits_at_operation_boundary() -> anyhow::Result<()> {
        ...
        use andrias_graph::OperationTemplate;
        let (facts, _) = FactSink::in_memory();
        let dp = DataPlane::new(
            Arc::new(RwLock::new(HandleTable::new())),
            facts,
        ...
        let prog = DoNode::op(OperationTemplate {
            ...
            method: "invoke".into(),
            method_id: None,
            output: andrias_types::OutputMode::Unary,
            literal_input: Some(Value::Null),
        });
        let out = ex.eval(&prog).await;
        ensure!(
            out == Outcome::Fail(andrias_types::Failure::Cancelled),
    ...
    async fn bound_handle_must_match_target_resource() -> anyhow::Result<()> {
        ...
        let out = ex
            ...
            matches!(
                out,
                Outcome::Fail(andrias_types::Failure::PolicyViolation { ref policy, .. })
                    if policy == "open"
            ),
    ...
    async fn step_ref_must_belong_to_current_process() -> anyhow::Result<()> {
        ...
        let out = ex.eval(&prog).await;
            ...
            matches!(out, Outcome::Fail(andrias_types::Failure::PolicyViolation { ref policy, .. }) if policy == "step"),
    ...
    async fn or_else_recovers() -> anyhow::Result<()> {
        ...
        let prog = DoNode::fail(andrias_types::Failure::Cancelled).or_else(s("fallback"));
        let out = ex.eval(&prog).await;
        ensure!(
    ...
    async fn both_fails_if_either_arm_fails() -> anyhow::Result<()> {
        ...
        let prog = DoNode::both(
            DoNode::pure(Value::Int(1)),
            DoNode::fail(andrias_types::Failure::Cancelled),
        );
        let out = ex.eval(&prog).await;
        ensure!(
    ...
    async fn wait_signal_resolves_on_write() -> anyhow::Result<()> {
        ...
        let signal = andrias_types::Path::parse("state://stream/1/sig")?;
    ...
    fn causal_positions_match_compiled_graph() -> anyhow::Result<()> {
        ...
        use andrias_graph::{NodeKind, compile_do};
        let prog = DoNode::op(andrias_graph::OperationTemplate {
            target: rn("effect://x/post")?,
            method: "invoke".into(),
            method_id: None,
            output: andrias_types::OutputMode::Unary,
    ...
    async fn protected_data_to_outbound_is_denied() -> anyhow::Result<()> {
        ...
        use andrias_graph::OperationTemplate;
        let ex = executor();
        let env = Env::root(IdentityRef::ROOT).with_taint(andrias_types::TaintSet::of(
            andrias_types::TaintSource::Protected {
                path: andrias_types::Path::parse("state://vault/alice/x")?,
            },
        ));
