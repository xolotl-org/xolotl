...
use andrias_graph::{ActorSpec, DoNode, LintSeverity, lint_actor};
use andrias_types::{
...
pub struct GatewayAudit<'a> {
    ...
    pub details: Option<andrias_types::Value>,
}
...
pub enum BootstrapError {
    ...
    State(#[source] Box<andrias_state::StateError>),
...
impl From<andrias_state::StateError> for BootstrapError {
    fn from(source: andrias_state::StateError) -> Self {
...
impl Bootstrap {
    ...
    pub fn register_effect(
        ...
        self.register_effect_with_cost(path, methods, driver, andrias_types::CostModel::default())
    }
    ...
    pub fn open_for(
        ...
        let resource_id = self
            ...
            .map_err(|_| OpenError::NoSuchResource(andrias_types::ResourceId::new(0)))?;
    ...
    fn normalize_selector_constraints(
        selector: &ResourceSelector,
    ) -> (ResourceSelector, Option<andrias_types::Predicate>) {
    ...
    fn derived_grant_constraints(
        parent: &Grant,
        requested_predicate: Option<andrias_types::Predicate>,
    ) -> ConstraintSet {
        ...
        I: IntoIterator<Item = ProcessStepBinding>,
    {
        ...
        andrias_graph::compile_do(&spec.body).map_err(|source| BootstrapError::ActorAdmission {
            actor: spec.name.clone(),
            message: source.to_string(),
        })?;
        for (index, finalizer) in spec.finalizers.iter().enumerate() {
            andrias_graph::compile_do(finalizer).map_err(|source| {
                BootstrapError::ActorAdmission {
                    actor: spec.name.clone(),
        ...
        let task = tokio::spawn(async move {
                ...
                .await
            {
                Ok(outcome) => outcome,
                Err(payload) => {
                    andrias_types::Outcome::Fail(andrias_types::Failure::HandlerError {
                        kind: "panic".into(),
            ...
            let status = match &outcome {
                andrias_types::Outcome::Fail(_) => ProcessStatus::Failed,
    ...
    pub fn record_gateway_audit(&self, audit: GatewayAudit<'_>) -> Result<(), crate::FactError> {
        ...
        outcome.insert(
            "event".into(),
            andrias_types::Value::Str(audit.event.into()),
        );
        outcome.insert(
            "outcome".into(),
            andrias_types::Value::Str(audit.outcome.into()),
        );
        if let Some(username) = audit.username {
            outcome.insert(
                "username".into(),
                andrias_types::Value::Str(username.into()),
            );
        }
        if let Some(source_addr) = audit.source_addr {
            outcome.insert(
                "source_addr".into(),
                andrias_types::Value::Str(source_addr.into()),
            );
        }
        if let Some(mfa_level) = audit.mfa_level {
            outcome.insert(
                "mfa_level".into(),
                andrias_types::Value::Int(i64::from(mfa_level)),
            );
        }
        ...
        self.kernel.facts.complete(Fact {
            id: andrias_types::OperationId::new(process, GATEWAY_AUDIT_NODE, 0),
            schema_version: Fact::SCHEMA_VERSION,
            caller: process,
            acting: IdentityRef::ROOT,
            handle: andrias_types::HandleId::new(0, 0),
            resource: andrias_types::ResourceId::new(0),
            method: andrias_types::MethodId::new(0),
            input_ref: andrias_types::ValueRef::Inline(andrias_types::Value::Null),
            taint: andrias_types::TaintSet::author(),
            decision: andrias_types::DecisionTag::Ok,
            outcome_ref: andrias_types::OutcomeRef::Inline(andrias_types::Value::Map(outcome)),
            batch: None,
            replay: andrias_types::ReplayClass::Observation,
            timestamp: andrias_types::Timestamp::millis(crate::executor::now_millis()),
        })?;
        self.kernel.processes.insert(entry);
        Ok(())
    }
    ...
    pub async fn finish_request_process(
        &self,
        process: ProcessId,
        outcome: &andrias_types::Outcome,
    ) -> Result<(), BootstrapError> {
        let status = match outcome {
            andrias_types::Outcome::Fail(
                andrias_types::Failure::Cancelled | andrias_types::Failure::Timeout,
            ) => ProcessStatus::Cancelled,
            andrias_types::Outcome::Fail(_) => ProcessStatus::Failed,
            andrias_types::Outcome::Done(_) | andrias_types::Outcome::Short(_) => {
...
const FINALIZED_NODE: andrias_types::NodeId = andrias_types::NodeId::new(u32::MAX);
const GATEWAY_AUDIT_NODE: andrias_types::NodeId = andrias_types::NodeId::new(u32::MAX - 1);
...
async fn finish_process_terminal(
        ...
        .enumerate()
    {
        ...
        if let andrias_types::Outcome::Fail(failure) = ex.eval(&body).await {
            ...
            item.insert(
                "index".into(),
                andrias_types::Value::Int((base_failure_index + index) as i64),
            );
            item.insert("failure".into(), andrias_types::Value::Str(failure));
            finalizer_failures.push(andrias_types::Value::Map(item));
        }
    }
    kernel
        .processes
    ...
    let finalized = Fact {
        id: andrias_types::OperationId::new(process, FINALIZED_NODE, 0),
        schema_version: Fact::SCHEMA_VERSION,
        caller: process,
        acting: kernel
            .processes
        ...
        decision: andrias_types::DecisionTag::Ok,
        outcome_ref: andrias_types::OutcomeRef::Inline(andrias_types::Value::Map({
            let mut m = BTreeMap::new();
            m.insert(
                "event".into(),
                andrias_types::Value::Str("ProcessFinalized".into()),
            );
            m.insert(
                "status".into(),
                andrias_types::Value::Str(process_status_label(terminal_status).into()),
            );
            m.insert(
                "revoked_handles".into(),
                andrias_types::Value::Int(revoked as i64),
            );
            m.insert(
                "finalizer_failure_count".into(),
                andrias_types::Value::Int(finalizer_failure_count as i64),
            );
            if !finalizer_failures.is_empty() {
                m.insert(
                    "finalizer_failures".into(),
                    andrias_types::Value::List(finalizer_failures),
                );
            }
            m
        })),
    ...
    let marker_result = kernel
        .state
        .write_set(&marker_path, andrias_types::Value::Int(revoked as i64))
        .await
...
fn actor_directory_value(
    ...
    let mut m = BTreeMap::new();
    m.insert("name".into(), andrias_types::Value::Str(spec.name.clone()));
    m.insert(
        "path".into(),
        andrias_types::Value::Str(format!("process://{}", process.get())),
    );
    m.insert(
        "process".into(),
        andrias_types::Value::Str(process.get().to_string()),
    );
    m.insert(
        "status".into(),
        andrias_types::Value::Str(process_status_label(status).into()),
    );
    m.insert("inbox".into(), andrias_types::Value::Str(inbox.to_string()));
    m.insert(
        "declared_capabilities".into(),
        andrias_types::Value::List(
            spec.declared_capabilities
                .iter()
                .cloned()
                .map(andrias_types::Value::Str)
                .collect(),
        ),
    );
    andrias_types::Value::Map(m)
}
...
fn method_bitmap_for_verb(
    ...
    resource_id: andrias_types::ResourceId,
    verb: &str,
) -> MethodBitmap {
...
fn build_methods(
    ...
    specs
        ...
        .map(|(i, spec)| Method {
            ...
            id: andrias_types::MethodId::new(i as u64),
...
mod tests {
    ...
    use andrias_graph::{ActorSpec, DoNode, OperationTemplate, StepRef};
    use andrias_types::{OutputMode, Value};
    ...
    async fn actor_spawn_runs_body_and_writes_directory() -> anyhow::Result<()> {
        ...
        let actor = boot
            .spawn_actor_under(boot.root, andrias_types::IdentityRef::ROOT, "root", &spec)
            .await?;
    ...
    async fn actor_spawn_rejects_missing_step_binding() -> anyhow::Result<()> {
        ...
        let err = expect_bootstrap_error(
            boot.spawn_actor_under(boot.root, andrias_types::IdentityRef::ROOT, "root", &spec)
                .await,
        )?;
        ensure!(
    ...
    async fn actor_spawn_with_step_binding_runs_immediately() -> anyhow::Result<()> {
        ...
        let actor = boot
            .spawn_actor_under_with_steps(
                boot.root,
                andrias_types::IdentityRef::ROOT,
                "root",
                &spec,
    ...
    async fn finalizer_failure_is_recorded_in_lifecycle_fact() -> anyhow::Result<()> {
        ...
        let spec = ActorSpec {
            ...
            body: DoNode::pure(Value::Null),
            finalizers: vec![DoNode::Fail(andrias_types::Failure::InvalidInput {
        ...
        let andrias_types::OutcomeRef::Inline(Value::Map(map)) = &finalized.outcome_ref else {
    ...
    async fn finalize_does_not_overwrite_terminal_actor_status() -> anyhow::Result<()> {
        ...
        let spec = ActorSpec {
            ...
            body: DoNode::Fail(andrias_types::Failure::InvalidInput {
    ...
    async fn actor_spawn_rejects_undeclared_capability() -> anyhow::Result<()> {
        ...
        let spec = ActorSpec {
            name: "bad_actor".into(),
            body: DoNode::op(OperationTemplate {
                target: andrias_types::ResourceName::new(andrias_types::Path::parse(
                    "effect://fetch/get",
                )?),
                method: "invoke".into(),
                method_id: None,
    ...
    async fn finalizer_mode_allows_only_current_process_state_subtree() -> anyhow::Result<()> {
        ...
        let local = ex
            ...
            matches!(local, andrias_types::Outcome::Done(_)),
        ...
        let sibling = ex
            ...
            matches!(
                sibling,
                andrias_types::Outcome::Fail(andrias_types::Failure::PolicyViolation { .. })
            ),
    ...
    async fn end_to_end_operation_flows_through_resolved_handle() -> anyhow::Result<()> {
        ...
        let out = ex.eval(&prog).await;
            ...
            out == andrias_types::Outcome::Done(Value::Str("hello".into())),
    ...
    async fn unary_only_method_rejects_stream_request() -> anyhow::Result<()> {
        ...
        match ex.eval(&prog).await {
            andrias_types::Outcome::Fail(andrias_types::Failure::InvalidInput { reason }) => {
                ensure!(
    ...
    async fn budget_exhaustion_denies_costly_op_before_effect() -> anyhow::Result<()> {
        ...
        match ex.eval(&prog).await {
            andrias_types::Outcome::Fail(andrias_types::Failure::BudgetExhausted { dim }) => {
                ensure!(
                    dim == "daily_micro_usd",
    ...
    async fn budget_settles_and_allows_within_limit() -> anyhow::Result<()> {
        ...
        let out = ex.eval(&prog).await;
            ...
            out == andrias_types::Outcome::Done(Value::Str("ok".into())),
    ...
    async fn consumed_operation_records_a_fact() -> anyhow::Result<()> {
        ...
        .and_then(andrias_graph::StepRef::new(boot.root, "echo_back"));
        let out = ex.eval(&prog).await;
        ensure!(
            out == andrias_types::Outcome::Done(Value::Str("hi".into())),
    ...
    async fn finalize_marks_completed_and_writes_marker() -> anyhow::Result<()> {
        ...
        boot.finalize_process(child).await?;
            ...
            boot.kernel.processes.status(child) == Some(andrias_types::ProcessStatus::Completed),
        ...
        let facts = boot.kernel.facts.facts_of(child)?;
            ...
            facts.iter().any(|f| {
                f.id.position == andrias_types::NodeId::new(u32::MAX)
                    && matches!(&f.outcome_ref, andrias_types::OutcomeRef::Inline(andrias_types::Value::Map(m))
    ...
    async fn finalize_preserves_cancelled_request_status() -> anyhow::Result<()> {
        ...
        boot.finalize_process(child).await?;
            ...
            boot.kernel.processes.status(child) == Some(andrias_types::ProcessStatus::Cancelled),
    ...
    impl crate::fact::FactStore for FailingFinalizeFactStore {
        ...
        fn facts_of(
            &self,
            _process: andrias_types::ProcessId,
    ...
    impl crate::fact::FactStore for FailOnceCompleteFactStore {
        ...
        fn facts_of(
            &self,
            process: andrias_types::ProcessId,
        ) -> Result<Vec<Fact>, crate::fact::FactError> {
    ...
    async fn finalize_fact_failure_keeps_process_finalizing() -> anyhow::Result<()> {
        ...
        let err = expect_bootstrap_error(boot.finalize_process(child).await)?;
            ...
            boot.kernel.processes.status(child) == Some(andrias_types::ProcessStatus::Finalizing),
    ...
    fn restricted_anchor(
        boot: &Bootstrap,
        selectors: &[&str],
    ) -> anyhow::Result<andrias_types::ProcessId> {
        let anchor = boot.kernel.processes.fresh_id();
        let mut entry =
            ProcessEntry::new(anchor, Some(boot.root), andrias_types::IdentityRef::ROOT);
        entry.status = ProcessStatus::Running;
    ...
    fn request_grant_derivation_preserves_parent_limits() -> anyhow::Result<()> {
        ...
        boot.kernel.registry.register_grant(Grant {
            ...
            rights: Rights::new(MethodBitmap::method(0), RightFlags::empty()),
            constraints: ConstraintSet {
                predicates: vec![andrias_types::Predicate::parse("account=alice")?],
            },
            expires,
        });
    ...
    async fn recovery_replays_completed_effect_without_reissuing() -> anyhow::Result<()> {
        ...
        let name = boot.register_effect(
            ...
            StdArc::new(FnDriver(|_m: andrias_types::MethodId, _in: Value| {
        ...
        .and_then(andrias_graph::StepRef::new(boot.root, "use_it"));
        ...
        let first = ex1.eval(&prog).await;
            ...
            first == andrias_types::Outcome::Done(Value::Int(7)),
        ...
        let out = ex2.eval(&prog).await;
            ...
            out == andrias_types::Outcome::Done(Value::Int(7)),
