...
pub struct ExecuteParams {
    ...
    pub replay: andrias_types::ReplayClass,
...
struct CompletedFact<'a> {
    op: &'a Operation,
    resource: andrias_types::ResourceId,
    replay: andrias_types::ReplayClass,
    now_millis: i64,
...
impl DataPlane {
    ...
    /// `replay` is the operation's derived [`andrias_types::ReplayClass`] used for the Fact
    ...
    async fn execute_inner(
        ...
        let idem_key = if matches!(replay, andrias_types::ReplayClass::IdempotentEffect) {
            let key = andrias_types::idempotency::derive_key(op.id, op.acting, &input);
            match self.read_idempotent_outcome(&key).await {
                Ok(Some(cached)) => {
                    ...
                    if (record
                        || matches!(
                            replay,
                            andrias_types::ReplayClass::IdempotentEffect
                                | andrias_types::ReplayClass::NonIdempotentEffect
                        ))
        ...
        if matches!(op.output, OutputMode::Stream) {
            match &result {
                Ok(_) => {
                    if !ctx.emit(Value::StreamEnd(andrias_types::StreamMarker::Done)) {
                        tracing::debug!(op = ?op.id, "stream end marker receiver closed");
                    }
                }
                Err(e) => {
                    if !ctx.emit(Value::StreamEnd(andrias_types::StreamMarker::Error {
                        message: e.to_string(),
                    })) {
    ...
    async fn read_idempotent_outcome(
        &self,
        key: &str,
    ) -> andrias_state::StateResult<Option<Outcome>> {
        let path = idempotency_path(key).map_err(|e| {
            andrias_state::StateError::Backend(format!("invalid idempotency path: {e}"))
        })?;
        match self.state.read(&path).await? {
            Some(value) => outcome_from_value(value)
                .map(Some)
                .map_err(andrias_state::StateError::Backend),
            None => Ok(None),
        }
    }
    ...
    fn deny(
        &self,
        op: &Operation,
        resource: Option<andrias_types::ResourceId>,
        replay: andrias_types::ReplayClass,
        now_millis: i64,
        tag: DecisionTag,
        failure: Failure,
        ...
        let fact = self.completed_fact(CompletedFact {
            op,
            resource: resource.unwrap_or_else(|| andrias_types::ResourceId::new(0)),
            replay,
            now_millis,
            decision: tag,
    ...
    fn completed_fact(&self, input: CompletedFact<'_>) -> Fact {
        ...
        let batch = if batchable {
            andrias_types::BatchSummary::new(&op.input, Some(&outcome_ref))
        } else {
            None
        };
        Fact {
    ...
    fn start_async_process(
        ...
        let child_handle = {
            ...
            match derive_handle(
                &mut handles,
                op.handle,
                andrias_types::Rights::new(
                    andrias_types::MethodBitmap::method(method_index),
                    andrias_types::RightFlags::empty(),
                ),
                andrias_types::DeriveKind::SpawnWith,
                child,
            ) {
                Ok(id) => id,
                Err(_) => {
        ...
        entry.status = andrias_types::ProcessStatus::Running;
        ...
        let (proc_path, status_path, outcome_path) = match (
            ...
            (proc_path, status_path, outcome_path) => {
                ...
                if processes
                    .mark_finalized_terminal(child, andrias_types::ProcessStatus::Failed)
                    .is_none()
                {
                    tracing::error!(
                        child = child.get(),
        ...
        let child_op = Operation {
            id: andrias_types::OperationId::new(child, andrias_types::NodeId::ROOT, 0),
            process: child,
            acting: op.acting,
            handle: child_handle,
            method: op.method,
        ...
        tokio::spawn(async move {
            ...
            let terminal = if outcome.is_success() {
                andrias_types::ProcessStatus::Completed
            } else {
                andrias_types::ProcessStatus::Failed
            };
            ...
            let phase = match actual_status {
                andrias_types::ProcessStatus::Completed => "completed",
                andrias_types::ProcessStatus::Failed => "failed",
                andrias_types::ProcessStatus::Cancelled => "cancelled",
                andrias_types::ProcessStatus::Created => "created",
                andrias_types::ProcessStatus::Running => "running",
                andrias_types::ProcessStatus::Waiting => "waiting",
                andrias_types::ProcessStatus::Suspended => "suspended",
                andrias_types::ProcessStatus::Finalizing => "finalizing",
            };
...
fn stream_path(op: &Operation) -> Result<Path, andrias_types::PathError> {
    Path::try_new("state")?
        .try_push("stream")?
...
fn collect_stream_path() -> Result<Path, andrias_types::PathError> {
    Path::try_new("state")?
        .try_push("stream")?
        .try_push("collect")
}

fn idempotency_path(key: &str) -> Result<Path, andrias_types::PathError> {
...
fn async_proc_path(process: andrias_types::ProcessId) -> Result<Path, andrias_types::PathError> {
    Path::try_new("proc")?
        .try_push("async")?
        .try_push_literal(process.get().to_string())
}

fn async_status_path(process: andrias_types::ProcessId) -> Result<Path, andrias_types::PathError> {
    Path::try_new("state")?
        .try_push("kernel")?
        .try_push("async")?
        .try_push_literal(process.get().to_string())?
...
fn async_outcome_path(process: andrias_types::ProcessId) -> Result<Path, andrias_types::PathError> {
    Path::try_new("state")?
        .try_push("kernel")?
        .try_push("async")?
        .try_push_literal(process.get().to_string())?
...
struct AsyncRefMeta {
    parent: andrias_types::ProcessId,
    child: andrias_types::ProcessId,
    resource: andrias_types::ResourceId,
    method: andrias_types::MethodId,
...
struct Resolved {
    ...
    fast_path: FastPath,
    bound_path: Option<andrias_types::Path>,
}
...
mod tests {
            ...
            match method.get() {
                ...
                4 => {
                    let rows = self
                        ...
                        .await
                        .map_err(|e| DriverError::Other(e.to_string()))?;
                    let mut taint = andrias_types::TaintSet::pristine();
                    let values = rows
                        .into_iter()
                        .map(|(p, tv)| {
    ...
    async fn state_read_output_taint_uses_persisted_taint() -> anyhow::Result<()> {
        ...
        let protected = andrias_types::TaintSet::of(andrias_types::TaintSource::Protected {
            path: path.clone(),
        });
        state
    ...
    #[async_trait::async_trait]
    impl andrias_state::StateBackend for FailingAppendState {
        async fn read_tainted(
            &self,
            _path: &Path,
        ) -> StateResult<Option<andrias_state::TaintedValue>> {
            Ok(None)
        }
        ...
        async fn read_prefix_tainted(
            &self,
            _prefix: &Path,
        ) -> StateResult<Vec<(Path, andrias_state::TaintedValue)>> {
            Ok(Vec::new())
        }
        ...
        let stream = state
            ...
            stream
                == Some(Value::List(vec![
                    ...
                    Value::StreamEnd(andrias_types::StreamMarker::Done),
                ])),
    ...
    async fn stream_append_failure_returns_driver_error() -> anyhow::Result<()> {
        ...
        let (facts, _) = FactSink::in_memory();
        let state: andrias_state::Backend = Arc::new(FailingAppendState);
        ...
        match out.outcome {
            Outcome::Fail(andrias_types::Failure::HandlerError { message, .. }) => {
                ensure!(
    ...
    async fn sink_only_suppresses_response_body() -> anyhow::Result<()> {
        ...
        let fact = facts.first().context("missing sink-only fact")?;
            ...
            matches!(
                fact.outcome_ref,
                andrias_types::OutcomeRef::Inline(Value::Null)
            ),
    ...
    async fn async_process_returns_pollable_resource_and_records_child_fact() -> anyhow::Result<()>
    {
        ...
        expected.insert("value".into(), Value::Str("work".into()));
            ...
            processes.status(child) == Some(andrias_types::ProcessStatus::Completed),
    ...
    async fn async_process_completion_does_not_overwrite_cancellation() -> anyhow::Result<()> {
        ...
        tokio::time::timeout(std::time::Duration::from_secs(1), async {
            loop {
                if let Some(andrias_types::ProcessStatus::Completed) = processes.status(child) {
        ...
        .context("async child did not publish cancelled status before timeout")??;
            ...
            processes.status(child) == Some(andrias_types::ProcessStatus::Cancelled),
