...
//! `andrias-kernel` - the Andrias runtime kernel.
//!
//! The control plane is a compiler: `open()` compiles a Grant against a
//! Resource into a [`Handle`]. The data plane is a VM that
//! executes [`Operation`](andrias_types::Operation)s through that handle's frozen
//! [`DriverPlan`], recording a [`Fact`](andrias_types::Fact).
//! The [`Executor`] advances an
//! [`ExecutionGraph`](andrias_graph::ExecutionGraph) cursor.
//!
//! Module map:
//!
