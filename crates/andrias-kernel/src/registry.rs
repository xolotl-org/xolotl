...
pub struct OpenCacheKey {
    ...
    pub resource_path: andrias_types::Path,
...
impl OpenCacheKey {
    ...
    pub fn new(
        grant: GrantId,
        resource: ResourceId,
        resource_path: andrias_types::Path,
        verb: impl Into<String>,
        rights: Rights,
        acting: andrias_types::IdentityRef,
        now_millis: i64,
    ) -> Self {
        Self {
            grant,
...
impl Registry {
    ...
    pub fn admit_resource(
        ...
        if !kernel_caller && andrias_types::is_kernel_reserved(name.path()) {
    ...
    pub fn check_namespace_sandbox(
        namespace: &andrias_types::Path,
        effect_paths: &[andrias_types::Path],
...
mod tests {
    ...
    fn namespace_sandbox_rejects_escape() -> anyhow::Result<()> {
        ...
        let ns = andrias_types::Path::parse("effect://external-provider/acme")?;
        let allowed = [
            andrias_types::Path::parse("effect://external-provider/acme/fetch")?,
            andrias_types::Path::parse("effect://external-provider/acme/post")?,
        ];
        ensure!(
        ...
        let escaped = [
            andrias_types::Path::parse("effect://external-provider/acme/ok")?,
            andrias_types::Path::parse("effect://x/post")?,
        ];
        ensure!(
            Registry::check_namespace_sandbox(&ns, &escaped).is_err(),
            "provider namespace should reject paths outside the namespace"
        );
        let sibling = [andrias_types::Path::parse(
    ...
    fn grants_of_filters_by_holder() -> anyhow::Result<()> {
        ...
        let g = |holder: u64, id: u64| -> anyhow::Result<Grant> {
            Ok(Grant {
                ...
                selector: andrias_types::ResourceSelector::parse("perform://effect/x")?,
                rights: andrias_types::Rights::default(),
                constraints: andrias_types::ConstraintSet::empty(),
                expires: andrias_types::Expiry::Never,
            })
        };
