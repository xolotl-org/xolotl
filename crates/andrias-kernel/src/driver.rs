...
/// Error a Driver may return. Distinct from [`andrias_types::Failure`]: a
...
pub struct DriverContext {
    ...
    pub acting: andrias_types::IdentityRef,
    /// The calling Process (sub-operations inherit this caller).
    pub caller: andrias_types::ProcessId,
    ...
    pub stream_to: Option<andrias_types::Path>,
    ...
    pub taint: andrias_types::TaintSet,
    ...
    pub target_path: Option<andrias_types::Path>,
    ...
    output_taint: Arc<Mutex<andrias_types::TaintSet>>,
}

impl DriverContext {
    /// Create a context for a call acting as `acting` on behalf of `caller`.
    pub fn new(acting: andrias_types::IdentityRef, caller: andrias_types::ProcessId) -> Self {
        Self {
            acting,
            caller,
            operation_id: None,
            ...
            target_path: None,
            stream_tx: None,
            output_taint: Arc::new(Mutex::new(andrias_types::TaintSet::pristine())),
        }
    }
    ...
    pub fn with_taint(mut self, taint: andrias_types::TaintSet) -> Self {
        self.taint = taint;
        self
    }

    /// Attach the concrete target path.
    pub fn with_target_path(mut self, path: andrias_types::Path) -> Self {
    ...
    /// output value.
    pub fn set_output_taint(&self, taint: andrias_types::TaintSet) {
        *self.output_taint.lock() = taint;
    }

    /// Return the output taint recorded by the driver.
    pub fn output_taint(&self) -> andrias_types::TaintSet {
...
pub struct DriverPlan {
    ...
    /// transport boundary.
    pub endpoint: Option<andrias_types::EndpointId>,
...
impl DriverPlan {
    ...
    pub fn new(
        driver_id: DriverId,
        endpoint: Option<andrias_types::EndpointId>,
        generation: u64,
    ) -> Self {
        Self {
            driver_id,
...
mod tests {
    use super::*;
    use andrias_types::{ErrorInfo, IdentityRef, NodeId, ProcessId};
    ...
    async fn remote_driver_sends_stable_invoke_frame() -> anyhow::Result<()> {
        ...
        let driver = RemoteDriver::new(
            EndpointId::new(7),
            andrias_types::ResourceId::new(11),
            3,
        ...
        let (dispatch, invoke) = seen.first().context("missing remote invoke")?;
            ...
            dispatch.resource_id == andrias_types::ResourceId::new(11),
