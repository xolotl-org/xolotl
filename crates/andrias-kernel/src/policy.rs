...
use andrias_types::{Capability, ConstraintSet, IdentityRef, Path, ResourceId, Rights, Value};
use std::sync::Arc;
...
impl RateLimitCheck {
    ...
    pub fn new(max_per_window: u32, window_millis: i64, state: andrias_state::Backend) -> Self {
        Self {
            max_per_window,
...
fn rate_limit_path(
    target: ResourceId,
    acting: IdentityRef,
) -> Result<Path, andrias_types::PathError> {
    Path::try_new("state")?
        .try_push("kernel")?
        .try_push("ratelimit")?
...
mod tests {
    ...
    use andrias_types::cap::Predicate;
    ...
    fn ctx<'a>(input: &'a Value) -> CheckCtx<'a> {
        CheckCtx {
            input,
            acting: andrias_types::IdentityRef::ROOT,
            now_millis: 0,
    ...
    async fn rate_limit_is_keyed_by_target_and_acting() -> anyhow::Result<()> {
        ...
        let alice = CheckCtx {
            input: &Value::Null,
            acting: andrias_types::IdentityRef::new(10),
            now_millis: 0,
            target: ResourceId::new(1),
        };
        let bob = CheckCtx {
            input: &Value::Null,
            acting: andrias_types::IdentityRef::new(20),
            now_millis: 0,
            target: ResourceId::new(1),
        };
    ...
    async fn rate_limit_persists_evicted_window() -> anyhow::Result<()> {
        ...
        let path = rate_limit_path(ResourceId::new(1), andrias_types::IdentityRef::ROOT)
    ...
    fn rate_limit_uses_documented_state_path() -> anyhow::Result<()> {
        let path = rate_limit_path(ResourceId::new(7), andrias_types::IdentityRef::new(9))
            .context("rate limit path failed")?;
        ensure!(
