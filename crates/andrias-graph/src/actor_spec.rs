...
use andrias_types::{BudgetSpec, Capability, PathError, Value};
...
impl ActorSpec {
    ...
    pub fn declares_capability(
        ...
        let path = andrias_types::Path::parse(target).map_err(|source| {
...
pub fn capability_covers(literal: &str, verb: &str, target: &str) -> bool {
    ...
    let Ok(path) = andrias_types::Path::parse(target) else {
        return false;
    };
...
mod tests {
    ...
    use andrias_types::{OutputMode, Path, ProcessId, ResourceName, Value};
