...
//! `andrias-graph` — the single execution IR and its front-ends.
//!
//! Every Program format (`Do<A>`, model plan, native) compiles to one
//! [`ExecutionGraph`]. The Executor (in `andrias-kernel`) advances a graph cursor
//! over that IR. Front-ends share the same executor and recovery model.
//!
//! This crate sits **below** the kernel and **above** `andrias-types`,
