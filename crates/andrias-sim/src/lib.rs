...
//! `andrias-sim` — deterministic simulation harness for tests and replay.
//!
...
use andrias_kernel::{Bootstrap, Driver, DriverContext, DriverError, DynDriver, FactSink};
use andrias_types::{MethodId, Outcome, OutputMode, ProcessId, Value};
...
impl Driver for ScriptedDriver {
    async fn call(
        ...
        Ok(self
            ...
            .lock()
            .pop_front()
            .unwrap_or(Outcome::Fail(andrias_types::Failure::Cancelled)))
    }
}
...
impl Sim {
    ...
    ) -> Result<andrias_types::ResourceName, andrias_kernel::BootstrapError> {
...
pub fn replay_report(
    facts: &FactSink,
    process: ProcessId,
) -> Result<andrias_kernel::RecoveryReport, andrias_kernel::FactError> {
    andrias_kernel::recover_process(facts, process).map(|(report, _, _)| report)
}
...
pub struct WhyNot {
    ...
    pub op: andrias_types::OperationId,
    /// The recorded decision tag (the proximate verdict).
    pub decision: andrias_types::DecisionTag,
...
pub fn why_not(facts: &[andrias_types::Fact], op: andrias_types::OperationId) -> Option<WhyNot> {
    use andrias_types::DecisionTag;
...
mod tests {
    ...
    use andrias_kernel::ProcessStepBinding;
    use andrias_types::OutputMode;
    use anyhow::{Context, bail, ensure};

    fn run_prog(name: andrias_types::ResourceName) -> DoNode {
        run_prog_with(name, Value::Null)
    }

    fn run_prog_with(name: andrias_types::ResourceName, input: Value) -> DoNode {
    ...
    fn is_process_finalized_fact(fact: &andrias_types::Fact) -> bool {
        matches!(
            &fact.outcome_ref,
            andrias_types::OutcomeRef::Inline(Value::Map(map))
    ...
    async fn injected_failure_routes_through_or_else_recovery_step() -> anyhow::Result<()> {
        ...
        driver.enqueue(Outcome::Fail(andrias_types::Failure::Cancelled));
        ...
        let fact = operation_facts
            ...
            fact.decision == andrias_types::DecisionTag::DriverError,
            "unexpected decision: {:?}",
            fact.decision
        );
        Ok(())
    ...
    async fn budget_denial_records_fact_without_calling_scripted_driver() -> anyhow::Result<()> {
        ...
        match out {
            Outcome::Fail(andrias_types::Failure::BudgetExhausted { dim }) => {
                ensure!(
                    dim == "daily_micro_usd",
        ...
        let facts = sim.boot.kernel.facts.facts_of(sim.boot.root)?;
            ...
            facts[0].decision == andrias_types::DecisionTag::RejectedByPolicy,
            "unexpected decision: {:?}",
            facts[0].decision
        );
        ensure!(facts[0].handle == handle, "unexpected fact handle");
        ensure!(
            facts[0].resource != andrias_types::ResourceId::new(0),
            "fact resource should be assigned"
        );
        ensure!(
            facts[0].replay == andrias_types::ReplayClass::NonIdempotentEffect,
    ...
    async fn idempotent_duplicate_short_circuits_driver_and_records_attempt() -> anyhow::Result<()>
    {
        ...
        let facts = sim.boot.kernel.facts.facts_of(sim.boot.root)?;
            ...
            facts[0].decision == andrias_types::DecisionTag::Ok,
            "unexpected decision: {:?}",
            facts[0].decision
        );
        ensure!(
            facts[0].replay == andrias_types::ReplayClass::IdempotentEffect,
            "unexpected replay class: {:?}",
            facts[0].replay
        );
        Ok(())
    ...
    async fn sim_clock_now_reflects_advance_and_set() -> anyhow::Result<()> {
        let ctx = DriverContext::new(andrias_types::IdentityRef::ROOT, ProcessId::new(1));
    ...
    async fn crash_after_n_stops_at_the_nth_call() -> anyhow::Result<()> {
        let ctx = DriverContext::new(andrias_types::IdentityRef::ROOT, ProcessId::new(1));
        let crasher = CrashAfter::arc(Arc::new(andrias_kernel::EchoDriver), 2);
