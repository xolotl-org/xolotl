...
//! Public data types for the Andrias runtime.
//!
...
//! live in `andrias-kernel`; the execution IR (`ExecutionGraph` / `Do<A>`) lives
//! in `andrias-graph`.
//!
//! Module map:
//!
...
mod workspace_contract_guard_tests {
    ...
    fn workspace_root() -> anyhow::Result<PathBuf> {
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            ...
            .context("andrias-types must live under crates/andrias-types")
    }
    ...
    fn data_plane_hot_path_has_no_control_plane_lookups_or_path_parsing() -> anyhow::Result<()> {
        let path = workspace_root()?.join("crates/andrias-kernel/src/dataplane.rs");
        let text = std::fs::read_to_string(&path)
