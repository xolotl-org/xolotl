...
//! - `Handle` (in `andrias-kernel`) — the compiled form produced by `open()`:
//!   a closed, executable, `O(1)` token. This is the "compiled product".
//!
//! `Grant` lives in `andrias-types` because it is pure data the control plane
...
/// `open()` (in `andrias-kernel`) compiles a Grant against a Resource into a
/// `Handle`.
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct Grant {
    /// Grant id.
