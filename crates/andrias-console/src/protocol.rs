...
//!
//! `andrias-console` is the control-plane protocol host. Web UI and third-party
...
pub const SERVER_NAME: &str = "andrias-console";
/// Default compact frame encoding advertised by the server.
pub const WIRE_ENCODING: &str = "msgpack+andrias-console-v1";
...
pub struct ActionCall {
    ...
    pub action: String,
    /// JSON-encoded Andrias value passed as action input.
    #[serde(default)]
    pub input: JsonBytes,
...
pub struct PrincipalSummary {
    /// Console username.
    pub username: String,
    /// Andrias identity path for the principal.
...
/// value envelope keeps Andrias's untagged `Value` representation explicit for
/// clients in any language.
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct JsonBytes(
    /// JSON string encoding a Andrias [`Value`].
    pub String,
);
...
impl JsonBytes {
    /// Serialize a Andrias value into the inner JSON string.
    pub fn try_from_value(v: &Value) -> Result<Self, serde_json::Error> {
        serde_json::to_string(v).map(Self)
    }

    /// Decode the inner JSON string into a Andrias value.
...
pub struct ActionResult {
    /// Optional JSON-encoded Andrias value output.
...
impl ActionResult {
    ...
    /// Construct an action result carrying one Andrias value.
...
/// Convert protocol metadata into a Andrias [`Value`].
...
/// Return one action descriptor encoded as a Andrias value.
...
/// Return one resource type descriptor encoded as a Andrias value.
...
/// Return one fixed resource view descriptor encoded as a Andrias value.
