...
use andrias_kernel::{Bootstrap, GatewayAudit};
use andrias_state::{Backend, StateError};
use andrias_types::{CapSet, Capability, Path, Value};
...
pub(crate) struct ConsolePrincipal {
    ...
    /// Andrias identity path associated with the user.
...
impl From<andrias_types::PathError> for AuthError {
...
fn key_login_transcript(username: &str, challenge_id: &str, nonce: &str, origin: &str) -> String {
    format!("andrias-console-ed25519-v1\n{username}\n{challenge_id}\n{nonce}\n{origin}")
}
...
mod tests {
    ...
    use andrias_types::{Fact, OutcomeRef};
