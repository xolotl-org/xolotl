...
use andrias_kernel::RequestGrantTemplate;
...
async fn run_state_op(
    ...
    let identity = andrias_kernel::intern_identity(&identity_path);
...
async fn authority_resource_access(
    ...
    if is_local_state && andrias_types::is_vault_reserved(&path) {
        return Ok(map_value([
...
impl From<andrias_types::PathError> for ConsoleError {
    fn from(e: andrias_types::PathError) -> Self {
        Self::BadRequest(e.to_string())
    }
}

impl From<andrias_state::StateError> for ConsoleError {
    fn from(e: andrias_state::StateError) -> Self {
        Self::Operation(e.to_string())
    }
}

impl From<andrias_kernel::FactError> for ConsoleError {
    fn from(e: andrias_kernel::FactError) -> Self {
        Self::Operation(e.to_string())
    }
}
...
fn facts_value(facts: Vec<andrias_types::Fact>) -> Value {
    Value::List(facts.into_iter().map(fact_value).collect())
}

fn fact_value(f: andrias_types::Fact) -> Value {
    let mut m = BTreeMap::new();
...
fn fact_detail_value(f: andrias_types::Fact) -> Result<Value, ConsoleError> {
...
fn registry_counts_value(counts: andrias_kernel::registry::RegistryCounts) -> Value {
    map_value([
...
fn ensure_observable_state_path(path: &Path) -> Result<(), ConsoleError> {
    ...
    if andrias_types::is_vault_reserved(path) {
        return Err(ConsoleError::BadRequest(
...
mod tests {
    ...
    use andrias_standard::{PairingDisplayEdge, StandardConfig, install_standard};
    use andrias_types::{
    ...
    impl andrias_kernel::FactStore for FailingFactStore {
        fn append(&self, _fact: andrias_types::Fact) -> Result<u64, andrias_kernel::FactError> {
            Err(andrias_kernel::FactError("simulated append failure".into()))
        }

        fn complete(&self, _fact: andrias_types::Fact) -> Result<(), andrias_kernel::FactError> {
            Err(andrias_kernel::FactError(
                "simulated complete failure".into(),
            ))
        }

        fn sync(&self) -> Result<(), andrias_kernel::FactError> {
            Ok(())
        }
        ...
        ) -> Result<Vec<andrias_types::Fact>, andrias_kernel::FactError> {
            Ok(Vec::new())
        }

        fn all_facts(&self) -> Result<Vec<andrias_types::Fact>, andrias_kernel::FactError> {
            Ok(Vec::new())
        }
    ...
    fn audit_outcomes(st: &ConsoleState, event: &str) -> anyhow::Result<Vec<String>> {
        Ok(st
            ...
            .filter_map(|fact| match fact.outcome_ref {
                andrias_types::OutcomeRef::Inline(Value::Map(m))
    ...
    fn root_principal() -> anyhow::Result<ConsolePrincipal> {
        Ok(ConsolePrincipal {
            ...
            grants: andrias_types::CapSet::from_strs(["*://**"])?,
            mfa_level: 2,
        })
    }
