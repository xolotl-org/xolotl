...
use xolotl_types::{BlobRef, MethodId, Outcome, OutputMode, Path, Value};
...
pub(crate) const BLOB_METHODS: &[MethodSpec] = &[
    ...
    MethodSpec::new("read", xolotl_types::Purity::Pure, MethodSpec::UNARY_ASYNC)
        .observes_external(),
    MethodSpec::new(
        "delete",
...
mod tests {
    ...
    fn blob_from(outcome: Outcome) -> anyhow::Result<xolotl_types::BlobRef> {
        match outcome {
