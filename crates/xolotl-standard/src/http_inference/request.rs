    ...
    match auth {
        ...
        HttpInferenceAuth::ApiKeyHeader { header, value } => {
            ...
            HeaderValue::from_str(value)
                .map_err(|_error| HttpInferenceError::BadHeader(header.clone()))?;
            Ok(())
        }
    }
}
...
fn validate_headers(headers: &BTreeMap<String, String>) -> Result<(), HttpInferenceError> {
    for (name, value) in headers {
        ...
        header_name(name)?;
        HeaderValue::from_str(value)
            .map_err(|_error| HttpInferenceError::BadHeader(name.clone()))?;
    }
    Ok(())
}
...
fn header_name(name: &str) -> Result<HeaderName, HttpInferenceError> {
    HeaderName::from_bytes(name.as_bytes())
        .map_err(|_error| HttpInferenceError::BadHeader(name.into()))
}

fn header_value(name: &str, value: &str) -> Result<HeaderValue, HttpInferenceError> {
    HeaderValue::from_str(value).map_err(|_error| HttpInferenceError::BadHeader(name.into()))
}

pub(super) fn endpoint_url(base_url: &str, path: &str) -> Result<String, HttpInferenceError> {
    let mut url = reqwest::Url::parse(base_url).map_err(|_error| HttpInferenceError::BadBaseUrl)?;
