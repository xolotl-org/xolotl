...
impl RankerDriver {
    ...
    pub(crate) fn with_state(mut self, state: nexus_state::Backend) -> Self {
    ...
    async fn resolve_weights(
        ...
        if let Some(state) = &self.state {
            let path =
                nexus_types::Path::parse("state://kernel/rank/weights").map_err(|error| {
...
mod tests {
    ...
    fn path(value: &str) -> Result<nexus_types::Path> {
        nexus_types::Path::parse(value).with_context(|| format!("parse {value}"))
    }
