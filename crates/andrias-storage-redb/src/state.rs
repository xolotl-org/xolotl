...
use andrias_types::{MergeRule, Path, TaintSet, Value};
...
/// redb-backed implementation of the Andrias `state://` backend.
...
impl StateBackend for RedbStateBackend {
    ...
    async fn write_merge(&self, path: &Path, value: Value, rule: MergeRule) -> StateResult<()> {
        ...
        let merged = andrias_state::merge_values(current, value, rule);
